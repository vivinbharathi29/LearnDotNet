...
''' for the display of System Board IDs 
	    ...
	    for each Row in Rowarray
		    if instr(Row,"^")=0 then
			    ...
			    if Ubound(IDArray) > 0 then
				    'Response.Write asc(left(IDArray(1),1))
...
<%
  ...
  strSQl = "SELECT " &_
                "p1.*, (SELECT CASE WHEN (p2.FusionRequirements = 1) THEN dbo.fnGetProductSystemBoardIDs(p1.ID) ELSE p2.SystemBoardComments END FROM ProductVersion p2 WHERE p2.ID = p1.ID) AS SystemboardComments " &_ 
                "FROM ( " & _
                "SELECT v.ID, v.DOTSName, p.Name AS Partner, v.SystemBoardID, dbo.Concatenate(b.SeriesSummary) AS SeriesSummary, dbo.Concatenate(COALESCE (bn.Name, '') + COALESCE (bn.Streetname, '') + COALESCE (bn.Suffix, '')) AS Brand, " & _ 
                 ...
                 "ORDER BY DOTSName"
'''' 12/22/2016, Herb, correct the display of system board.
...
<br /><font size=2 face=verdana><b>Fields searched:</b>  ID, Name, Series, Partner, Brand
...
%>
	<tr LANGUAGE=javascript onclick="return ProductMenu(<%=rs("ID")%>)">
		...
		<td class="cell"><%=FormatSystemID(rs("SystemboardComments") & "")%>&nbsp;</td>
...
<%
	...
	do while not rs.EOF
        ...
        RowCount = RowCount + 1	
		Response.Write "<TR><TD valign=top><a target=OTS href=""http://" & Application("Excalibur_ServerName") & "/Excalibur/search/ots/Report.asp?txtReportSections=1&txtObservationID=" & rs("ID") & """><font size=1 face=verdana>" & rs("ID") & "</a></td>"
...
<%
    'response.Flush
    '
    ' Search ODM Part Number
    '
    strPartSql = "select distinct HpPartNo, OdmPartNo, OdmPartDesc, partner.name as OdmName, categoryname " & _
                "from servicesparedetail with (NOLOCK) " & _
                "left outer join servicesparecategory with (NOLOCK) on servicesparedetail.sparecategoryid = servicesparecategory.id " & _
                "left outer join partner with (NOLOCK) on servicesparedetail.partnerid = partner.id " & _
                "where odmpartno like '" & strParts &"'"

 
    rs.ActiveConnection = cn  
    rs.Open  strPartSql ,cn, adOpenForwardOnly
	if rs.EOF and rs.EOF then
		Response.Write "<font face=verdana size=3><b>No ODM Part Information Found in the Pulsar Data</b></font><br /><br />"
		Response.Write "<font size=2 face=verdana><b>Fields searched:&nbsp;&nbsp;</b>ODM Part Number"
		Response.write "<br /><b>Search Criteria:&nbsp;&nbsp;</b> " & replace(strParts,"''","'") &  "</font>"
	else

%>
  
<font face=verdana size=3><b>ODM Part Information Found in Pulsar</b></font><br />
<font size=2 face=verdana><b><br />Fields searched:&nbsp;&nbsp;</b>Part Number
<br /><b>Search Criteria:&nbsp;&nbsp;</b><%=replace(strParts,"''","'")%></font><br /><br />
<table id="Table9" bgColor=ivory borderColor=tan width="95%" border=1 cellspacing=1 cellpadding=2>
    <tr bgColor=cornsilk>
		<td><b>HP Part No</b></td>
		<td><b>ODM Part No</b></td>
		<td><b>ODM Description</b></td>
		<td><b>ODM</b></td>
		<td><b>Spare Category</b></td>
	</tr>
...
<%
	...
	do while not rs.EOF
		...
		Response.Write "<tr><td valign=top><font size=1 face=verdana><a href=""find.asp?type=Part&find=" & Server.urlencode(Trim(rs("HpPartNo"))) & """>" & rs("HpPartNo") & "</a>&nbsp;</td>"
		Response.Write "<td valign=top>" & rs("ODMPartNo") & "&nbsp;</td>"
		Response.Write "<td valign=top>" & rs("OdmPartDesc") & "&nbsp;</td>"
		Response.Write "<td valign=top>" & rs("OdmName")  & "&nbsp;</td>"
		Response.Write "<td valign=top>" & rs("categoryname") & "&nbsp;</td>"
		Response.Write "</tr>"
...
'
  ...
  rs.Open "SELECT r.ID,r.name, r.active, s.base as baseSubassembly, v.dotsname as Product, s.subassembly from deliverableroot r with (NOLOCK), product_delroot s with (NOLOCK), productversion v with (NOLOCK) Where v.id = s.productversionid and r.id = s.deliverablerootid and (s.base like  '%" & strParts & "%' or s.subassembly like  '%" & strParts & "%' ) "
	if rs.EOF and rs.EOF then
		Response.Write "<font family=verdana size=4><b>No Subassemblies Found in Pulsar Data</b></font><br /><br />"
...
%>
<font face=verdana size=3><b>Subassemblies Found in Pulsar Data</b></font><br />
<br /><font size=2 face=verdana><b>Fields searched:</b>  Base Subassembly Number, Subassembly Number
<br /><b>Search Criteria:&nbsp;</b><%=server.HTMLEncode(strFindScrubbed) %></font><br />
<table id="TABLE5" bgColor=ivory borderColor=tan WIDTH="95%" BORDER=1 CELLSPACING=1 CELLPADDING=2 LANGUAGE=javascript onmousemove="return rows_onmousemove()" onmouseout="return rows_onmouseout()">
<% 
	ColorIndex = 1
	EOLRowCount=0
	HeaderDrawn = false
...
'
	...
	if rs.EOF and rs.EOF then 
		Response.Write "<font family=verdana size=4><b>No Deliverable Versions Found in Pulsar Data</font></b><br /><br />"
		Response.Write "<font size=2 face=verdana><b>Fields searched:</b>  Part Number, IRS Part Number , CD Part Number, and Softpaq Number"
		Response.write "<br /><b>Search Criteria:</b> " & server.HTMLEncode(strFindScrubbed) &  "</font><br />"
		rs.Close
	else
%>
...
<font face=verdana size=3><b>Deliverable Versions Found in Pulsar Data</b></font><br />
<br /><font size=2 face=verdana><b>Fields searched:</b> Part Number, IRS Part Number, CD Part Number, Softpaq Number
<br /><b>Search Criteria:</b>&nbsp;<%=server.HTMLEncode(strFindScrubbed)%></font><br />
<table id="TABLE6" bgColor=ivory borderColor=tan WIDTH="95%" BORDER=1 CELLSPACING=1 CELLPADDING=2 LANGUAGE=javascript onmousemove="return rows_onmousemove()" onmouseout="return rows_onmouseout()">
<% 
	ColorIndex = 1
	EOLRowCount=0
	HeaderDrawn = false
...
%>
    <tr bgcolor=cornsilk>
		...
		<td><b>Workflow</b></td>
		<TD style=display:none><b>Path</b></td>
	</tr>
    ...
    <TR LANGUAGE=javascript onclick="return DeliverableAlertMenu(<%=rs("RootID")%>,<%=rs("ID")%>,1)"> 
		<TD class="cell"><%=rs("ID")%></td>
		...
		<TD  style=display:none class="cell" ID="Path1_<%=trim(rs("ID"))%>"><%=rs("ImagePath")%>&nbsp;</td>
	</TR>
...
<%  end if 'not active
	...
	end if 'if rs.EOF and rs.EOF then
    ...
    'response.Flush
    '
    ' Search Pulsar Images
    '
    strPartSql = "select pv.id as ProductVersionID, d.id as ImageDefinitionID, i.skunumber, pv.dotsname, os.Name as osname, r.Geo, r.name as regionname, r.dash, r.OptionConfig " & _
        "from imagedefinitions d with (NOLOCK) inner join images i with (NOLOCK) on i.imagedefinitionid = d.id " & _
        "inner join oslookup os with (NOLOCK) on d.osid = os.id " & _
        "inner join productversion pv with (NOLOCK) on d.productversionid = pv.id " & _
        "inner join regions r with (NOLOCK) on i.regionid = r.id " & _
        "where i.skunumber like '%" & strParts & "%' " & _
        "and d.active = 1 "
 
    rs.ActiveConnection = cn  
    rs.Open  strPartSql ,cn, adOpenForwardOnly
	if rs.EOF and rs.EOF then
		Response.Write "<font face=verdana size=3><b>No Images Found in Pulsar Data</b></font><br /><br />"
		Response.Write "<font size=2 face=verdana><b>Fields searched:&nbsp;&nbsp;</b>SKU Number"
		Response.write "<br /><b>Search Criteria:&nbsp;&nbsp;</b> " & replace(strParts,"''","'") &  "</font>"
	else
%>
  
<font face=verdana size=3><b>Images Found in Pulsar Data</b></font><br />
<font size=2 face=verdana><b><br />Fields searched:&nbsp;&nbsp;</b>SKU Number
<br /><b>Search Criteria:&nbsp;&nbsp;</b><%=replace(strParts,"''","'")%></font><br /><br />
<table id="Table7" bgColor=ivory borderColor=tan width="95%" border=1 cellspacing=1 cellpadding=2>
    <tr bgColor=cornsilk>
...
<%
	...
	do while not rs.EOF
        ...
        RowCount=RowCount + 1
		Response.Write "<TD valign=top><a target=new href=""/image/localization2.asp?ImageID=" & rs("ImageDefinitionID") & "&PINTest=0&ProdID=" & rs("ProductVersionID") & """>" & rs("skunumber") & "</a>&nbsp;</td>"
		Response.Write "<TD valign=top>" & rs("DotsName") & "&nbsp;</td>"
		Response.Write "<TD valign=top>" & rs("OsName") & "&nbsp;</td>"
		Response.Write "<TD valign=top>" & rs("regionname") & "&nbsp;</td>"
		Response.Write "<TD valign=top>" & rs("optionconfig")  & "&nbsp;</td>"
		Response.Write "</TR>"
