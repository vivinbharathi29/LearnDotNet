...
<%
	...
	if not (rs.EOF and rs.BOF) then
		strEngineer = rs("SCFactoryEngineerID") & ""
	...
	dim strEngineerList
	strEngineerList = ""
	rs.Open "spListFactoryEngineersAll",cn,adOpenForwardOnly
	do while not rs.EOF
		if trim(strEngineer) = trim(rs("ID")) then
			strEngineerList = strEngineerList & "<OPTION selected value=" & rs("ID") & ">" & rs("Name") & "</OPTION>"
		else
			strEngineerList = strEngineerList & "<OPTION value=" & rs("ID") & ">" & rs("Name") & "</OPTION>"
		end if
		rs.MoveNext
	loop
	rs.Close	
...
<FORM ACTION="ProgramFactoryEngineerSave.asp" METHOD="post" id="ProgramInput">

<table ID=tabEngineer border="1" cellPadding="2" cellSpacing="0" width="100%" bgcolor=cornsilk bordercolor=tan>
  
   <tr>
    <td width="160" style="VERTICAL-ALIGN: top"><strong><font size=2>Factory&nbsp;Engineer:</font></strong></td>
    <td><SELECT id=cboEngineer name=cboEngineer style="WIDTH: 180px;" LANGUAGE=javascript onkeypress="return combo_onkeypress()" onfocus="return combo_onfocus()" onclick="return combo_onclick()" onkeydown="return combo_onkeydown()">
