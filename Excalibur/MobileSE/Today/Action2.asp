...
<% 
	...
	elseif 1=1 then 'ConfigErrors = "" then
            ...
            <% dim urlNew 
                urlNew =  "/PulsarPlus/product/product/GetTodayAction/" & IssueID & "/" & TypeID & "/0/0"
                %>
           <% Response.Redirect urlNew%>
			<!--<FRAMESET ROWS="*,55" ID=TopWindow>

			    <FRAME ID="UpperWindow" frameborder="0" Name="UpperWindow" SRC="actionmain.asp?ProdID=<%=ProdID%>&CAT=<%=CategoryID%>&ID=<%=IssueID%>&Type=<%=TypeID%>">
			    <FRAME ID="LowerWindow" frameborder="0" Name="LowerWindow" SRC="actionbuttons.asp?ProdID=<%=ProdID%>&CAT=<%=CategoryID%>&ID=<%=IssueID%>&Type=<%=TypeID%>" scrolling=no>
