	...
	if rs.EOF and rs.BOF then
		...
		do while not rs.EOF
		    ...
		    if trim(rs("PostRTMStatus")) = "0" then
    			getProduct = getProduct & rs("ID") & "*" & "R" & "*" & rs("Name") & " " & rs("Version") & "*" & rs("AllowSMR") & "*" &  rs("SEPMID") & "*" & strStreetNames & "^"
		    elseif trim(rs("PostRTMStatus")) = "1" then
    			getProduct = getProduct & rs("ID") & "*" & "C" & "*" & rs("Name") & " " & rs("Version") & "*" & rs("AllowSMR") & "*" &  rs("SEPMID") & "*" & strStreetNames & "^"
		    end if
		    rs.MoveNext
		loop
	end if
