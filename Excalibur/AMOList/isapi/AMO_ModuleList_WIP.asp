...
dim nCategoryID, nNumTotalModules, sGpsyCom, sRasCom, sHideColumns
...
if sErr = "" then
	...
	sStatusIDs = cstr(Application("AMO_NEW")) & "," & cstr(Application("AMO_REJECT")) & "," _
			 & cstr(Application("AMO_DISABLED")) & "," & cstr(Application("AMO_RE-ENABLED")) _
			  & "," & cstr(Application("AMO_IN_PROCESS"))
...
'	end if
	...
	sErr = GetFilterInfo(nCategoriesSelected, sStatusIDs, sBusSegIDs, strEOLDate,_
			 sModuleCategoryHTML, sStatusHTML, oRsAMOModules, _
			  ...
			  bDisplayHideromMOL, bDisplayHideromSCM, true, strMRFromDate, strMRToDate, _
...
function ClickEvent(evt) {
	...
	switch (id){
		...
		case 'as': //AMO status column
			...
			break;
		case 'asdis': //Lhuy:AMO status column=disabled
			cM_Status(evt, ModuleID, '')
			break;
	    ...
	    case 'sf': //sug 9881 
	        ...
	        break;
		case 'idp': //Ingore Deployment Plan - 4-29-2008 Added by Dien
			if (objUnknown.innerHTML == "&nbsp;") {	
				// add checkmark				
				cM_ChangeIDP(evt, ModuleID, 1, objUnknown, repository, fullname);				
			} else {
				// remove checkmark
				cM_ChangeIDP(evt, ModuleID, 0, objUnknown, repository, fullname);
			}
			break;
	}
}
...
function enterComment(ModuleID, Field) {
	...
	thisform.action = "AMO_AddComment.asp?stab=WIP";
	thisform.submit ();
}
...
function lbxGo_onhide() {
	thisform.action = "AMO_ViewCustomize.asp?nParent=AMO_ModuleList_WIP";
	thisform.target = "_self"
	thisform.submit ();
}
...
if sErr <> "" then
	...
	<TABLE border=0 cellPadding=1 cellSpacing=1 width=100%>
		<% Call WriteCategoryFilter( "WIP", sModuleCategoryHTML, sStatusHTML, sBusSegHTML, strEOLDate,_
	...
	<TABLE id="tblAMOList" border=0 cellPadding=1 cellSpacing=1 width="100%">
		...
		if nNumTotalModules <= 0 then
			...
			<%
			if bRAS then
				WriteRASUpdateableGridHTML  "WIP", oRsAMOModules, sDivisionIDs, sGroupIDs, sHideColumns
			else
				WriteAMOUpdateableGridHTML  "WIP", oRsAMOModules, sDivisionIDs, sGroupIDs, sHideColumns
			end if
			%>
			</td></tr>
			<tr><td>
