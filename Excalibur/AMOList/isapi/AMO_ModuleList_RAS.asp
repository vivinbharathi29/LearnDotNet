...
if request.form("ID") <> "" and request.form("StatusID") <> "" then
	...
	elseif clng(request.form("StatusID")) = clng(Application("AMO_REJECT")) and bRASUpdate then
		...
		if strError <> "True" then
			...
			call SendStatusChangeEmail (False,True,clng(Application("AMO_REJECT")),Request.Form("ID"),Request.Form("RegionID"))
		end if
	end if
end if
...
if sErr = "" then
	...
	sStatusIDs = cstr(Application("AMO_RASUPDATE")) & "," & cstr(Application("AMO_RASREVIEW"))
...
<title><%=sHeader%> - RAS Review</title>
...
function ClickEvent(evt) {
	...
	switch (id){
		...
		case 'asras': //AMO status column for RAS Review modules
			cM_Status(evt, ModuleID, 'ras', RegionID)
			break;
...
function enterComment(ModuleID, Field) {
	...
	thisform.action = "AMO_AddComment.asp?stab=RAS";
	thisform.submit ();
}
...
function cM_Status(evt, ModuleID, sStatus, RegionID) {
	var menu1=new Array()
	var wide = 120;
	
	if (sStatus == 'ras') {
		<% if bRAS then %>
		menu1.push("<a href='../nj.asp' onClick=\"ChangeStatus(" + ModuleID + ", <%= Application("AMO_REJECT") %>, " + RegionID + "); return false;\">Reject</a>");
		<% end if%>
	} else {
...
function ChangeStatus(ModuleID, StatusID, RegionID) {
	var msg
	...
	if (confirm(msg)) {
		...
		thisform.RegionID.value = RegionID
		thisform.action = "AMO_ModuleList_RAS.asp?nMode=1";
...
function lbxGo_onhide() {
	thisform.action = "AMO_ViewCustomize.asp?nParent=AMO_ModuleList_RAS";
	thisform.target = "_self"
	thisform.submit ();
}
...
function BulkstatusChange() {
    ...
    for (var i=0;i< collStatus.length; i++) {
        if (collStatus[i].checked) {
            ...
            strAllRegionIDs = strAllRegionIDs + "," + arrModId_Status[3];
            ...
            if (collActions.value == "1") {
                ...
                if (arrModId_Status[1] == "RAS Update" || arrModId_Status[1] == "RAS Review") {				
                    strModuleIDs = strModuleIDs + "," + arrModId_Status[0]; 
                    strRegionIDs = strRegionIDs + "," + arrModId_Status[3];
                }   
            }
        }
    }
    if (strModuleIDs != "") {
        ...
        strRegionIDs = strRegionIDs.slice(1);
    }
	...
	if (collActions.value == "1") {	
		...
		if (confirm(msg)) {
			...
			if (errormsg == "success") {
				thisform.action = "AMO_ModuleList_WIP.asp?nMode=1";
				thisform.target = "_self"
				thisform.submit ();
			}								
		}
		...
		if (confirm(msg)) {	
			...
			if (errormsg == "success") {
			    ajaxurl = "AMO_SetBulkStatusEmailValue.asp?RGS=1&ModuleID=" + strModuleIDs + "&RegionID=" + strRegionIDs;
                //objRS.setBulkStatusEmailValue(strModuleIDs,strRegionIDs,<%=Application("AMO_REJECT")%>);
...
function btnCreateExportFile_onclick() {
	thisform.action = "AMO_GenerateExport.asp?Mode=3";
	thisform.submit ();
	return true;
}
//-->
...
if sErr <> "" then
	...
	<TABLE border=0 cellPadding=1 cellSpacing=1 width=100%>
		<% Call WriteCategoryFilter( "RAS" & "&nbsp;" & "Review", sModuleCategoryHTML, sStatusHTML, sBusSegHTML, strEOLDate,_
		...
		<% if bAMOCreate then %>
			<INPUT type='button' value='Create AMO Modules' style="width:150px" id=btnAdd NAME=btnAdd LANGUAGE=javascript onClick="return btnAdd_onClick();">&nbsp;&nbsp;
	...
	<TABLE id="tblAMOList" border=0 cellPadding=1 cellSpacing=1 width="100%">
		...
		if nNumTotalModules <= 0 then
			...
			<%
			if bRAS then
				WriteRASUpdateableGridHTML "RAS" & "&nbsp;" & "Review", oRsAMOModules, sDivisionIDs, sGroupIDs, sHideColumns
			else
				WriteAMOUpdateableGridHTML "RAS" & "&nbsp;" & "Review", oRsAMOModules, sDivisionIDs, sGroupIDs, sHideColumns
			end if
			%>
			</td></tr>
			<tr><td>
