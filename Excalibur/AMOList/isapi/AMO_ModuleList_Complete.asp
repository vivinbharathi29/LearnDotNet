...
dim bRASCreate, bRASView, bRASUpdate, bRASDelete, sHideColumns
...
if sErr = "" then
	...
	strDisFromDate = cstr(month(now()))  & "/" & cstr(day(now())) & "/" & cstr(year(now()))
	...
	sErr = GetFilterInfo(nCategoriesSelected, sStatusIDs, sBusSegIDs, strEOLDate,_
			 sModuleCategoryHTML, sStatusHTML, oRsAMOModules, _
			  ...
			  "","","","",strDisFromDate,"","checked","checked","", _
...
function enterComment(ModuleID, Field) {
	...
	thisform.action = "AMO_AddComment.asp?stab=Complete";
	thisform.submit ();
}
...
function ChangeStatus(ModuleID, StatusID) {
	...
	if (confirm(msg)) {
		...
		thisform.action = "AMO_ModuleList_Complete.asp?nMode=1";
...
function lbxGo_onhide() {
	thisform.action = "AMO_ViewCustomize.asp?nParent=AMO_ModuleList_Complete";
	thisform.target = "_self"
	thisform.submit ();
}
...
function ChangeRAS_GPSy(nChkIndex, ModuleID) {
	...
	if (RASItem.checked == true && GPSyItem.checked == true) {
		var msg = "You are about to change the status of this Option to Complete. Do you wish to continue?"
		if (confirm(msg)) {
		    bSaveEntry = 1;
		    bSetComplete = 1;
		} else {
...
if sErr <> "" then
	...
	<TABLE border=0 cellPadding=1 cellSpacing=1 width=100%>
		<% Call WriteCategoryFilter( "Complete", sModuleCategoryHTML, sStatusHTML, sBusSegHTML, strEOLDate,_
		 ...
		 <%if sHideColumns <> "" then%>
			<tr><td colspan='3'><font color=red>Warning: some column is hidden, click on "Select Column To Hide" to view hidden columns</font></td></tr>
		<%end if %>
		<tr><td colspan=2>
	...
	<TABLE border=0 cellPadding=1 cellSpacing=1 width="100%">
		...
		if nNumTotalModules <= 0 then
			...
			<%
			if bRAS then
				WriteRASUpdateableGridHTML "Complete", oRsAMOModules, sDivisionIDs, sGroupIDs, sHideColumns
			else
				WriteAMOUpdateableGridHTML "Complete", oRsAMOModules, sDivisionIDs, sGroupIDs, sHideColumns
			end if
			%>
			</td></tr>
			<tr><td>
...
<table>
	...
	<tr>
     ...
     <tr>
		<td >Only AMO Features with "Complete" Status and an End of Manufacturing date are included.</td>
	</tr>
	<tr>
