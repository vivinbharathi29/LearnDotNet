...
if sBusSegIDs = "" then
	sErr = "No Business Segment values passed, AMO_CreateBlindDateFile.asp"
	Response.Write(sErr)
    Response.End()
end if
...
if sErr = "" then
    ...
    set oSvr = New ISAMO
	set  oRsOptions = oSvr.AMO_GetBlindDateData(Application("REPOSITORY"), sBusSegIDs, sOwnerIDs, nUserID)
end if

if sErr = "" then
	Response.clear()
	Response.ContentType = "text/plain"
	Response.AddHeader "content-disposition", "attachment; filename=AMO_SelectAvailability.txt"

	response.write oRsOptions("UserName") & vbCrLf
	response.write "MSK" & vbTab
		response.write "DTE" & vbTab & "M" & vbTab & "PROD_NBR" & vbTab 
		response.write "NEW_EFF_DT" & vbTab & "OLD_EFF_DT" & vbTab & "REVIEW_TYPE_CD" & vbCrLf
	
	do while not oRsOptions.EOF
		response.write "DTE" & vbTab & oRsOptions("M") & vbTab & oRsOptions("PROD_NBR") & vbTab 
		response.write makeDateFormat(oRsOptions("New_Eff_DT")) & vbTab & makeDateFormat(oRsOptions("Old_Eff_DT")) & vbTab & "NEW" & vbCrLf
