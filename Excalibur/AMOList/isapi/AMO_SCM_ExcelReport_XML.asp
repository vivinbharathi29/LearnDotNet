...
set rsBS = GetCategory(44, Session("AMOUserID"))	
...
while len(sSelectedBusSegIDs) > 0 
	...
	rsBS.Filter = "DivisionID = " & sSelectedDivisionID
	
	if rsBS.recordCount > 0 then
		if sUserBusSeg = "" then
			sUserBusSeg = rsBS("Division").value
		else
			sUserBusSeg = sUserBusSeg & " , " & rsBS("Division").value
		end if	
	end if
wend
...
if len(Request.QueryString("SCMID")) > 0 and Request.QueryString("SCMID")<> "1" then
	...
	sFilter = " and (R.SCMID = " & SCMID & " Or R.SCMID = null) and O.SCMID = " & SCMID	& " and o.HidefromSCM=0 and R.RASDiscontinueDate is not null"
	...
	if strEOLDate <> "" then
		sFilter = sFilter & " and (R.moduleID in (select distinct moduleID from AMO_Region where SCMID = 1 And RASDiscontinueDate >= '" & strEOLDate & "')) and R.RASDiscontinueDate is not null"
	else
		sFilter = sFilter & " and R.RASDiscontinueDate is not null"
	end if
...
if len(bsnapshot) > 0 then
	...
	if len(sModuleIDS) > 0 then
		set sErr = oSvr.AMO_SCM_Publish(Application("REPOSITORY"), session("FullName"), sModuleIDS,SCMID )
		if not oErr is nothing then
			sErr = "Missing required parameters.  Unable to complete your request."
		    Response.Write(sErr)
		    Response.End()
		else
			Response.Redirect "AMO_Reports.asp?nMol=" & nMol & "&SCMID=" & SCMID & "&BusSeg =" & sBusSegIDs
		end if	
	else	
...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
 ...
 <Worksheet ss:Name="AMO_SCM Report">
  ...
  <Table x:FullColumns="1"
   ...
   <%
	if oRsAMOModules.recordcount> 0 then
		WriteAMO_SCM_ReportGridXML oRsAMOModules, SCMID, PublishDate, sBusSegIDs, sUserBusSeg
	else
