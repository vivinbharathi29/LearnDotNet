...
{   if (isEmpty(s)) 
    ...
    var num = Number(s);    
    //var num = parseInt (s); //This doesn't seem to work always
    return ((num >= a) && (num <= b));
}
...
/*
Need to change this so it works in Firefox and IE.
...
function warnInvalid (theField, s) {
	alert(s)
	setTimeout(function(){theField.focus();theField.select();},10); // needed for Firefox
	return false;
}
...
// Make sure there is at most N numbers after the decimal point
// isFloat should be done first to ensure the number is valid
// added by JEC 5/28/02
function MaxDecNumbers (s, maxnumbers) {
	var i;
	var intCount;
	var seenDecimalPoint = false;
	...
	for (i = 0; i < s.length; i++) {   
		...
		if (c == decimalPointDelimiter) {
			...
			if (intCount == maxnumbers) return false
			else intCount += 1;
		}			
	}
...
// Make sure there is at most N numbers before the decimal point
// isFloat should be done first to ensure the number is valid
// added by JEC 11/5/03
function MaxBeforeDecNumbers (s, maxnumbers) {
	var i;
	var intCount = 0;
...
// Return True if there are any whitespace characters in the string including empty, otherwise False
// added by JEC 4/23/04
...
// then to trim a string just use [string_name].trim();
// added by WAT 10/01/04
String.prototype.trim = function() {
   return this.replace(/(^\s*)|(\s*$)/g, "");
}

// Use a regular expression to replace &nbsp; characters with the empty string
// then to trim a string just use [string_name].trimNBSP();
// added by WAT 10/01/04
String.prototype.trimNBSP = function() {
   return this.replace(/&nbsp;/g, "");
}

// Use a regular expression to replace HTML tags with the empty string
// then to trim a string just use [string_name].trimHTML();
// added by WAT 10/01/04
...
function validateNotEmpty(strValue) {
	...
	if(its_not_visible(strValue)){
		return false;
	}

	strTemp = strTemp.trim();
	if(its_not_visible(strTemp)){
		return false;
   }
		return true;
}

// DESCRIPTION: Validates that a string is not all blank (whitespace) nor HTML tags
// PARAMETERS: strValue - String to be tested for validity
// RETURNS: True if valid, otherwise false.
// added by WAT 10/01/04
function validateNotEmptyHTML(strValue) {
	var strTemp = strValue
	...
	strTemp = strTemp.trimNBSP();
	strTemp = strTemp.trimHTML();
	strTemp = strTemp.trim();
	if(its_not_visible(strTemp)){
		return false;
   }
