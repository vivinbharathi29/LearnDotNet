...
<%response.Flush %>
<font size="3" face="verdana"><b>Sync Images with Product Explorer</b></font><br />
<form id="frmMain" action="SelectProductImageUpdatesSave.asp" method="post" target="_blank" >
<input id="Submit1" type="submit" value="Send Selected Changes to Product Explorer" /><br /><br />
<%
    ...
    dim lastPartNo
    dim productDropName
    dim lastPulsarProductDrop
    dim productName
    dim LeadProductVersionID
    dim LeadResultList
    dim LeadProd 
	...
	if trim(request("ProductDrop")) = "" and trim(request("ProductID")) = "" and trim(request("ImageDefinitionID")) = "" then
		...
		elseif trim(request("ProductDrop")) <> "" then 'Product Drop Supplied
            ...
            rs.open "SELECT distinct ProductVersionID from ProductVersion_Release WHERE ID=" & clng(request("ProductID")), cn
            if not rs.eof then
                LeadProductVersionID = CInt(rs("ProductVersionID"))
            else
                LeadProductVersionID = 0
            end if
            rs.close

			rs.open "spListProductDrops4product " & LeadProductVersionID & ", 1",cn
			do while not rs.eof
                if instr(rs("ProductDrop") & "","##") > 0  and (rs("StatusID") = 1) then
	...
	for i = 0 to ubound(productdroparray)
		if trim(Productdroparray(i)) = "" then
            ...
            ProductVersionID = CInt(tmparray(1))
            productName = tmparray(2)
            
            if (lastPulsarProductDrop <> productDropName) then
                LeadResultList=""
                strSQL = "SELECT ProductDropID from ProductDrop where Name ='" & FixProductDrop(trim(productDropName)) & "'"
			    ...
			    if strProductDropID <> "" then
				    ...
				    response.write "</td></tr><tr><td colspan=12>Product Drop Not Found in Product Explorer</td></tr>"
			    end if
			    rs2.Open "usp_Image_ListImagesForProductDrop '" & productDropName & "'",cn,adOpenForwardOnly
			    strImageList = ""
		       ...
		       'response.write strImageList & "<br>" 
			    strDelIRS = ""
			    strSQl = "spListDeliverablesInProductExplorer '" & FixProductDrop(productDropName) & "'"
			    rs2.Open strSQl,cn,adOpenForwardOnly
			    do while not rs2.EOF
                    ...
                    if Not IsNull(rs2("DMIStr")) and rs2("DMIStr") <> "" then
					    installOptionList = installOptionList & "2|" & FixProductDrop(productDropName) & "|" & trim(rs2("PartNo")) & "|" & Left(rs2("DMIStr"), len(rs2("DMIStr"))-1) & "|" 
				    end if
                    ...
                    if Not IsNull(rs2("Regions")) then
					    regionsList = regionsList & "2|" & FixProductDrop(productDropName) & "|" & trim(rs2("PartNo")) & "|"  & rs2("Regions") & "|" '& vbcrlf
				    end if
                    if Not IsNull(rs2("Locales")) then
					    localesList = localesList & "2|" & FixProductDrop(productDropName) & "|" & trim(rs2("PartNo")) & "|"  & rs2("Locales") & "|" '& vbcrlf
				    end if
				    strDelIRS = strDelIRS & vbcrlf
				    rs2.MoveNext
			    loop
			    rs2.Close
			    lastPulsarProductDrop = productDropName
            end if


			strSQl = "spListDeliverablesInProductDrop '" & productDropName & "'," &  ProductVersionID & ",1" 
			rs2.Open strSQl,cn,adOpenForwardOnly
			strOutbuffer = ""	
			do while not rs2.EOF
            ...
            if (ProductVersionID = LeadProductVersionID) then
                LeadProd = true
            else
                LeadProd = false
            end if
    ...
    if RowOutputCount = 0 then
        response.write "<tr><td colspan=12>No changes needed.</td></tr>"
    elseif LinesDisplayedTotal > 50 then
    	response.write "</table><br /><input id=""Submit2"" type=""submit"" value=""Send Selected Changes to Product Explorer"" />"
    end if
...
function getRegion(searchStr)
		...
		if (Not IsNull(regionsList) and Len(regionsList) > 0 ) then
            ...
            if (strStartIndex <> strEndIndex) then
			    getRegion = mid(regionsList,strStartIndex,(strEndIndex-1-strStartIndex))
...
function getLocale(searchStr)
		...
		if (Not IsNull(localesList) and Len(localesList) > 0 ) then
            ...
            if (strStartIndex <> strEndIndex) then
			    getLocale = mid(localesList,strStartIndex,(strEndIndex-1-strStartIndex))
            end if 
		end if
end function
...
function InArray(MyArray,strFind)
	...
	function OutputLine
						...
						if ExcaliburCount = 0 and IRSCount > 0 then 'Remove
							...
							for each strPartNumber in PNArray
                                if (ProductVersionID = LeadProductVersionID) then
                                    LeadResultList = LeadResultList + "|" + strPartNumber + ":Rem"
                                end if
                                if LeadProd or instr(LeadResultList, strPartNumber + ":Rem") = 0 then
                                    response.write "<tr>"
								    rs2.open "spGetProductAndDeliverableInfoForIRSFeed " & ProductVersionID & ",'" & strPartNumber & "'",cn
								    if not (rs2.eof and rs2.bof) then
									    if trim(strProductDropID) <> "" and (Not IsNull(rs2("VersionID"))) then
										    response.write "<td><input name=""ChangeDetails"" class=""PD" & strProductDropID & """ type=""checkbox"" value=""" & rs2("ProductName") & "|" & FixProductDrop(productDropName) & "|" & strProductDropID & "|" & replace(rs2("DeliverableName"),",","^") & "|rem|" & strPartNumber & "|" & cstr(rs2("VersionID")) & "|" & cstr(rs2("SRP")&"") & "|" & cstr(rs2("NotInRCD")&"") & "|" & cstr(rs2("InAppRecovery")&"") & "|" & cstr(rs2("DIB")&"") & "|" & """ onclick=""javascript: RemoveItem(checked)"" />Remove</td>"
									        remCount = remCount + 1
									    else
						...
						elseif ExcaliburCount > 0 and IRSCount = 0 then 'Add
							...
							for each strPartNumber in PNArray
                                 if (ProductVersionID = LeadProductVersionID) then
                                    LeadResultList = LeadResultList + "|" + strPartNumber + ":Add"
                                 end if
                                 if LeadProd or instr(LeadResultList, strPartNumber + ":Add") = 0 then
                                    response.write "<tr>"
								    rs2.open "spGetProductAndDeliverableInfoForIRSFeed " & ProductVersionID & ",'" & strPartNumber & "'",cn
								    if not (rs2.eof and rs2.bof) then
									    if trim(strProductDropID) <> "" and (Not IsNull(rs2("VersionID"))) then
										    response.write "<td><input name=""ChangeDetails"" class=""PD" & strProductDropID & """ type=""checkbox"" value=""" & rs2("ProductName") & "|" & FixProductDrop(productDropName) & "|" & strProductDropID & "|" & replace(rs2("DeliverableName"),",","^") & "|add|" & strPartNumber & "|" & cstr(rs2("VersionID")) & "|" & cstr(rs2("SRP")) & "|" & cstr(rs2("NotInRCD")) & "|" & cstr(rs2("InAppRecovery")) & "|" & cstr(rs2("DIB")) & "|"" onclick=""javascript: AddItem(checked)"" />Add</td>"
                                            addCount = addCount + 1
									    else
    ...
    '                                    response.write "<td>" & rs2("ProductName") & "|" & FixProductDrop(Productdroparray(i)) & "|" & strProductDropID & "|" & replace(rs2("DeliverableName"),",","^") & "|add|" & strPartNumber & "|" & cstr(rs2("IRSComponentPassID")) & "|" & cstr(rs2("SRP")) & "|" & cstr(rs2("NotInRCD")) & "|" & cstr(rs2("InAppRecovery")) & "|" & cstr(rs2("DIB")) & "|</td>" 
						...
						elseif ExcaliburCount =1 and IRSCount =1 then 'Replace
							...
							ExcaliburPNList = replace(ExcaliburPNList,"|","")
                             if (ProductVersionID = LeadProductVersionID) then
                                    LeadResultList = LeadResultList + "|" + ExcaliburPNList + ":Rep"
                             end if
                             if LeadProd or instr(LeadResultList, ExcaliburPNList + ":Rep") = 0 then
                                response.write "<tr>"
                                rs2.open "spGetProductAndDeliverableInfoForIRSFeed " & ProductVersionID & ",'" & ExcaliburPNList & "'",cn
							    if not (rs2.eof and rs2.bof) then
								    if trim(strProductDropID) <> "" and (Not IsNull(rs2("VersionID"))) then
									    response.write "<td><input  name=""ChangeDetails"" class=""PD" & strProductDropID & """ type=""checkbox"" value=""" & rs2("ProductName") & "|" & FixProductDrop(productDropName) & "|" & strProductDropID & "|" & replace(rs2("DeliverableName"),",","^") & "|rep|" & ExcaliburPNList & "|" & cstr(rs2("VersionID")) & "|" & cstr(rs2("SRP")) & "|" & cstr(rs2("NotInRCD")) & "|" & cstr(rs2("InAppRecovery")) & "|" & cstr(rs2("DIB")) & "|" & IRSPNList & """ onclick=""javascript: ReplaceItem(checked)""  />Replace</td>"
								        repCount = repCount + 1
								    else
    ...
    '                                response.write "<td>" & rs2("ProductName") & "|" & FixProductDrop(Productdroparray(i)) & "|" & strProductDropID & "|" & replace(rs2("DeliverableName"),",","^") & "|rep|" & ExcaliburPNList & "|" & cstr(rs2("IRSComponentPassID")) & "|" & cstr(rs2("SRP")) & "|" & cstr(rs2("NotInRCD")) & "|" & cstr(rs2("InAppRecovery")) & "|" & cstr(rs2("DIB")) & "|" & IRSPNList & "</td>"
						...
						elseif ExcaliburCount > 0 and IRSCount > 0 then 'Add All and Remove All
							...
							for each strPartNumber in PNArray
								...
								if not (rs2.eof and rs2.bof) then
									if trim(strProductDropID) <> "" and (Not IsNull(rs2("VersionID"))) then
										response.write "<td><input name=""ChangeDetails"" class=""PD" & strProductDropID & """ type=""checkbox"" value=""" & rs2("ProductName") & "|" & FixProductDrop(productDropName) & "|" & strProductDropID & "|" & replace(rs2("DeliverableName"),",","^") & "|rem|" & strPartNumber & "|" & cstr(rs2("VersionID")) & "|" & cstr(rs2("SRP")&"") & "|" & cstr(rs2("NotInRCD")&"") & "|" & cstr(rs2("InAppRecovery")&"") & "|" & cstr(rs2("DIB")&"") & "|"" />Remove</td>"
									else
										response.write "<td>Remove&nbsp;</td>"
									end if
'                                    response.write "<td>" & rs2("ProductName") & "|" & FixProductDrop(Productdroparray(i)) & "|" & strProductDropID & "|" & replace(rs2("DeliverableName"),",","^") & "|rem|" & strPartNumber & "|" & cstr(rs2("IRSComponentPassID")) & "|" & cstr(rs2("SRP")&"") & "|" & cstr(rs2("NotInRCD")&"") & "|" & cstr(rs2("InAppRecovery")&"") & "|" & cstr(rs2("DIB")&"") & "|</td>" 
							...
							for each strPartNumber in PNArray
								...
								if not (rs2.eof and rs2.bof) then
									if trim(strProductDropID) <> "" and (Not IsNull(rs2("VersionID"))) then
										response.write "<td><input name=""ChangeDetails"" class=""PD" & strProductDropID & """ type=""checkbox"" value=""" & rs2("ProductName") & "|" & FixProductDrop(productDropName) & "|" & strProductDropID & "|" & replace(rs2("DeliverableName"),",","^") & "|add|" & strPartNumber & "|" & cstr(rs2("VersionID")) & "|" & cstr(rs2("SRP")) & "|" & cstr(rs2("NotInRCD")) & "|" & cstr(rs2("InAppRecovery")) & "|" & cstr(rs2("DIB")) & "|"" />Add</td>"
									else
										response.write "<td>Add</td>"
									end if
								   ' response.write "<td>" & rs2("ProductName") & "|" & FixProductDrop(Productdroparray(i)) & "|" & strProductDropID & "|" & replace(rs2("DeliverableName"),",","^") & "|add|" & strPartNumber & "|" & cstr(rs2("IRSComponentPassID")) & "|" & cstr(rs2("SRP")) & "|" & cstr(rs2("NotInRCD")) & "|" & cstr(rs2("InAppRecovery")) & "|" & cstr(rs2("DIB")) & "|</td>" 
									...
									strDeliverableName = strDeliverableName & "]"
									response.write  "<td>" & rs2("ProductName") & "</td><td>" & strDeliverableName & "</td><td>" & ucase(strPartNumber) & "</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td>"
