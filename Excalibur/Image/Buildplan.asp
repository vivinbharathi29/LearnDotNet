...
function EditFCS(){
	var strRC;
	strRC = window.showModalDialog("FCSEdit.asp?ID=" + txtID.value,"","dialogWidth:700px;dialogHeight:600px;edge: Raised;center:Yes; help: No;resizable: Yes;status: No"); 
...
<%
  ...
  'Create a recordset
	...
	'rs.Open "spGetRampPlanUpdateAccessList '" & request("ID") & "'",cn,adOpenForwardOnly
	do while not rs.EOF	
...
'	rs.open "spGetProductVersionName " & request("ID"),cn,adOpenForwardOnly
	if rs.EOF and rs.BOF then
		...
		if request("SW") <> "" then
			strFilters = strFilters & ", " & request("SW")
		end if
		if request("Geo") <> "" then
		...
		if strFilters <> "" then
			...
			Response.Write "<font size=2 color=black face=verdana><b>Filtered By:</b> " & strFilters & "&nbsp;&nbsp;<a href=""http://" & Application("Excalibur_ServerName") & "../image/fusion/Buildplan.asp?ID=" & Request("ID") & """><font size=1>Show All</font></a><BR><BR></font>"
		end if
        ...
        if trim(request("SW")) <> "" then
    		Response.Write "<Th>Software</Th>"
	    else
    		Response.Write "<Th onclick=""SortTable( 'plantable', 5 ,0,2);"" onmouseout=""javascript: HeaderMouseOut();"" onmouseover=""javascript: HeaderMouseOver();"">Software</Th>"
        end if
        if trim(request("Image")) <> "" then
        	Response.Write "<Th>Image</Th>"
        else	
            Response.Write "<Th onclick=""SortTable( 'plantable', 6 ,0,2);"" onmouseout=""javascript: HeaderMouseOut();"" onmouseover=""javascript: HeaderMouseOver();"">Image</Th>"
        end if	
		...
		Set cm.ActiveConnection = cn
		cm.CommandType = 4
		cm.CommandText = "spListImagesForProductRollout"
        ...
        RowCount = 0
		'rs.Open "spListImagesForProductRollout " & request("ID"),cn,adOpenForwardOnly
		do while not rs.EOF


			strDash = rs("Dash")& ""
			
			strImage = ucase(rs("SKUNumber") & "")
			if not isnumeric(rs("Priority")) then
				strImage = replace(strImage,"-XX",replace(ucase(trim(rs("Priority")& "")),"X",""))
			else
				strImage = replace(strImage,"-XX",replace(ucase(rs("Dash")& ""),"X",""))
			end if
			...
			elseif (trim(rs("Priority")) = "10" and strP10 <> "") then
				...
				Set rs2 = cm.Execute 
				Set cm=nothing
'				if trim(rs("Priority")) = "1" or trim(rs("Priority")) = "2" or trim(rs("Priority")) = "3" or trim(rs("Priority")) = "4" or trim(rs("Priority")) = "5" then
...
'				rs2.open strSQL,cn,adOpenForwardOnly
			...
			if  (request("Geo") = rs("Geo") or  request("Geo") = "") and (request("SW") = rs("SW") or  request("SW") = "") and (request("Brand") = rs("Brand") or  request("Brand") = "") and (request("Opsys") = rs("OS") or  request("Opsys") = "") and (request("Region") = rs("Region") or  request("Region") = "") and (request("Image") = strImage or  request("Image") = "") and (request("Dash") = strDash or  request("Dash") = "") and (request("FCS") = strFCS or  request("FCS") = "" or ( strFCS="&nbsp;" and request("FCS") = "TBD" ) )and (request("Actual") = strActual or  request("Actual") = "" or ( strActual="&nbsp;" and request("Actual") = "TBD" ))and (request("HPCode") = strHPCode or  request("HPCode") = "" or ( strHPCode="&nbsp;" and request("HPCode") = "TBD" ))and (request("FAISKU") = strFAISKU or  request("FAISKU") = ""  or ( strFAISKU="&nbsp;" and request("FAISKU") = "TBD" ))and (request("Comments") = strComments or  request("Comments") = "" or ( strComments="&nbsp;" and request("Comments") = "TBD" )) and (request("Priority") = strPriority or  request("Priority") = "") and (instr(strLanguage,request("Language"))>0 or  request("Language") = "") then
				...
				if request("Brand") = rs("Brand") then
					Response.Write "<TD nowrap class=selected>" & rs("Brand") & "</TD>"
				else
					Response.Write "<TD nowrap class=normal><a class=Filter href=""javascript:FilterMatrix(1,'" & rs("Brand") & "')"">" & rs("Brand") & "</a></TD>"
				end if
				...
				if request("SW") = rs("SW") then
					Response.Write "<TD nowrap class=selected>" & rs("SW") & "</TD>"
				else
					Response.Write "<TD nowrap class=normal><a class=Filter href=""javascript:FilterMatrix(13,'" & rs("SW") & "')"">" & rs("SW") & "</a></TD>"
				end if
