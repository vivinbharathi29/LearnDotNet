...
'	rs.open "spGetProductVersionName " & request("ID"),cn,adOpenForwardOnly
	...
	'	end if
        ...
        if trim(request("Image")) <> "" then
            ...
            Response.Write "<Th onclick=""SortTable( 'plantable', 5 ,0,2);"" onmouseout=""javascript: HeaderMouseOut();"" onmouseover=""javascript: HeaderMouseOver();"">Product&nbsp;Drop</Th>"
        end if	
        if trim(request("Dash")) <> "" then
        	Response.Write "<Th>Dash Code</Th>"
	    else
        	Response.Write "<Th onclick=""SortTable( 'plantable', 6 ,0,2);"" onmouseout=""javascript: HeaderMouseOut();"" onmouseover=""javascript: HeaderMouseOver();"">Dash Code</Th>"
        end if
    	ColCount = 7
		if Request("Report") <> "1" then
		...
		Set cm.ActiveConnection = cn
		cm.CommandType = 4
		cm.CommandText = "spListImagesForProductRolloutFusion"
