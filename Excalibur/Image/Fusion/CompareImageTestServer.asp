...
<b><font face=verdana size=x-small color=red>Test Server</font></b><br><br>
...
'			else
'				strConveyor = "16.159.144.23"'"tpopsgcvr3.auth.hpicorp.net"
...
'			end if
		...
		'	rs.Open "spListImagesForProductAll " & request("ProdID"),cn,adOpenForwardOnly
			...
			if request("ImageDefinitionID") <> "" or request("lstImageDefinitions") <> ""  then
				...
				Response.Write "<DIV ID=ReportTitle style=""display:none""><font size=3 face=verdana><b><center>" &  strCompareType & "</b></font><BR><BR><font size=2 face=verdana>Compare Excalibur Image Definitions to the " & strServerLocation & " Conveyor Images</font></center><BR>"
				Response.Write "<font size=2 face=verdana><u><b>Results</b></u></font><BR><BR></div>"
			end if
			...
			'Excalibur deliverables have been loaded into strOutBuffer for this image 
				...
				cn2.ConnectionString = "Provider=SQLOLEDB.1;Password=123456;Persist Security Info=True;User ID=viewer;Initial Catalog=cvrbeta;Data Source=" & strConveyor & ";Use Procedure for Prepare=1;Auto Translate=True;Packet Size=4096;Workstation ID=KB2;"
				cn2.Open
				'cn2.CommandTimeout = 300
				
				rs2.Open "select max(skurevision) as Rev from sku with (NOLOCK) where skunumber = '" & ScrubSQL(strSKU) & "' and locked <> -11;",cn2,adOpenForwardOnly
				...
				if strSKURevision <> "" then
					'Converoy query1
					strSQl = "select delivname as DeliverableName, delivversion as Version, delivrevision as Revision, delivextpass as Pass " & _
							 "from osldeliv with (NOLOCK), bomosl with (NOLOCK), bom with (NOLOCK), sku with (NOLOCK) " & _
							 "where skunumber = '" & ScrubSQL(strSKU) & "' " & _
							"and skurevision = '" & ScrubSQL(strSKURevision) & "' " & _
							"and sku.bomkey = bom.bomkey " & _
							"and bom.bomkey = bomosl.bomkey " & _
							"and bomosl.bomoslkey = osldeliv.bomoslkey " & _
							"and linking = 0 " & _
							"order by delivname, delivversion, delivrevision, delivextpass;"
                    'response.Write strSQL & "--------<BR>"
        
					rs2.Open strSQl,cn2,adOpenForwardOnly
					do while not rs2.EOF
						strDelConveyor = strDelConveyor & "2" & trim(rs2("DeliverableName")) & " " &  rs2("Version")
						if  rs2("Revision") & "" <> "" then
							strDelConveyor = strDelConveyor & " " & rs2("Revision")
						end if
						if lcase(rs2("Pass")) >="a" and lcase(rs2("Pass")) <= "z" then
							strDelConveyor = strDelConveyor & asc(lcase(rs2("Pass"))) -87
						else
							strDelConveyor = strDelConveyor & rs2("Pass")
						end if
					...
					'Converoy query2
					strSQl = "select langcode,a.delivname as DeliverableName, a.delivversion as Version, a.delivrevision as Revision, a.delivextpass as Pass " & _ 
							 "from assemblydeliv a with (NOLOCK), assembly b with (NOLOCK), osldeliv c with (NOLOCK), bomosl with (NOLOCK), bom with (NOLOCK), sku with (NOLOCK) " & _
							 "where skunumber = '" & ScrubSQL(strSKU) & "' " & _
							 "and skurevision = '" & ScrubSQL(strSKURevision) & "' " & _
							 "and sku.bomkey = bom.bomkey " & _
							 "and bom.bomkey = bomosl.bomkey " & _
							 "and bomosl.bomoslkey = c.bomoslkey " & _
							 "and linking = 1 " & _
							 "and c.delivdivcode = b.divcode " & _
							 "and c.delivname = b.assemblyname " & _
							 "and c.delivoscode = b.oscode " & _
							 "and c.delivlangcode = b.langcode " & _
							 "and c.delivversion = b.version " & _
							 "and c.delivrevision = b.revision " & _
							 "and c.delivextpass = b.extpass " & _
							 "and c.delivintpass = b.intpass " & _
							 "and b.assemblykey = a.assemblykey " & _
							 "order by a.delivname, a.delivversion, a.delivrevision, a.delivextpass;"
					rs2.Open strSQl,cn2,adOpenForwardOnly
...
%>
<TABLE border=0 bordercolor=Indigo cellspacing=1 cellpadding=2>
<TR><TD class=SummaryTH>Targeted Versions Not In Images:</b></td><TD class=SummaryTD>&nbsp;
<%if strMissingTargetCount > 0 then%>
...
<%
    ...
    set cn = server.CreateObject("ADODB.Connection")
    set rs = server.CreateObject("ADODB.Recordset")
	cn.ConnectionString = "Provider=SQLOLEDB.1;Password=123456;Persist Security Info=True;User ID=viewer;Initial Catalog=CVRBETA;Data Source=" & strConveyor & ";Use Procedure for Prepare=1;Auto Translate=True;Packet Size=4096;Workstation ID=KB2;"
	cn.Open
    ...
    do while not rs.eof
        if not InArray(ExcaliburSKUArray,rs("skunumber") & "") then
            if not ExtraConveyorHeaderDisplayed then
                response.write "<BR><b><font size=2 face=verdana>SKUs in Conveyor that are not defined in Excalibur for this product.</b></font>"
