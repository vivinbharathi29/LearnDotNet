...
%>
	...
	dim OSLinks
	OSLinks=""
	rs.Open "[usp_Image_ListImageDefinitionsOSByProduct] " & cint(request("ProdID")) & ",3",cn,adOpenStatic	
	do while not rs.eof
		if rs("ID") <> "56" and rs("ID") <> "30" and rs("ID") <> "83" then
...
%>
	...
	<INPUT style="display:none" type="text" id=PINTest name=PINTest value="<%=request("PINTest")%>">
	    <input style="display:none" id="chkServer" onclick="javascript:ChangeServer();" type="checkbox" /> 	
	</form>	
<%
	end if
