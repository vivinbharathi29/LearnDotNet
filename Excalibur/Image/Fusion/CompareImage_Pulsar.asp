    ...
    for i = 0 to ubound(productdroparray)
        if trim(Productdroparray(i)) = "" then
          ...
          '  response.write "+++" & Productdroparray(i)  & "<BR>"
            rs2.Open "usp_Image_ListImagesForProductDrop '" & Productdroparray(i) & "'",cn,adOpenForwardOnly
            strImageList = ""
           ...
           ' response.write strImageList & "<br>"
            ...
            '    strSQl = "spListComponentsInIRSImage '" & FixProductDrop(productdroparray(i)) & "',1"
            'else
            '    strSQl = "spListComponentsInIRSImage '" & FixProductDrop(productdroparray(i)) & "',0"
            'end if
            strSQl = "spListComponentsInIRSImage '" & FixProductDrop(productdroparray(i)) & "'"

            rs2.Open strSQl,cn,adOpenForwardOnly
		    do while not rs2.EOF
			    strDelIRS = strDelIRS & "2" & trim(rs2("DotsName")) & " " &  trim(rs2("Version"))
            ...
            if true then 'lcase(Session("LoggedInUser")) = "auth\dwhorton1" then
        ...
        elseif ErrorCount = 0 then
            ...
            response.Write "</table></form></div><br><br>"
...
function InArray(MyArray,strFind)
    ...
    '---------  Delete from here down when finished -----------
if false then    
...
'    end if
    ...
    function InArray2(MyArray, strValue)
        dim i
        dim blnFound 
        blnFound = false
