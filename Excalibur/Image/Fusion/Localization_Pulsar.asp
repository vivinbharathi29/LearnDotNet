 ...
 ' Response.ExpiresAbsolute = Now() - 1
  ...
  'Response.CacheControl = "no-cache"
	...
	if request("FileType")= 1 then
        ...
        Response.AddHeader "content-disposition", "attachment;filename=Localization_Pulsar.xls"
	elseif request("FileType")= 2 then
		Response.ContentType = "application/msword"
        Response.AddHeader "content-disposition", "attachment;filename=Localization_Pulsar.doc"
	end if
...
'		rs.Open "spGetProductVersionByName '" & request("Product") & "'",cn,adOpenForwardOnly
    ...
    if request("ReleaseName") <> "" then
		strFilterList = strFilterList & ",ReleaseName:" & request("ReleaseName") 
	 end if
	...
	if strFilterList <> "" then
		...
		if request("FileType") = "" then
			strCountNote = strCountNote & "<a href=""Localization_Pulsar.asp?ProdID=" & DisplayedID & """>Show All</a>"
		end if
...
<%
	...
	dim Tier9Found
	dim Tier10Found
    dim strReleaseNameRow
    dim strOSReleaseNameRow
	dim boolRegionsDisplay
    ...
    if request("ProductVersionReleaseID") = "" and request("ReleaseName") = "" then
		strReleaseNameRow = "<TR BGCOLOR=white>"
	else
		strReleaseNameRow = "<TR BGCOLOR=" & strFilterColor & ">"
	end if

    strTierCountRow = "<TR BGCOLOR=white>"
	...
	if request("ImageID") <> "" then
		...
		Set cm.ActiveConnection = cn
		cm.CommandType = 4
		cm.CommandText = "usp_Image_GetImageDefinitionBrands"
		...
		Set cm.ActiveConnection = cn
		cm.CommandType = 4
		cm.CommandText = "usp_Image_ListImageDefinitions4ImageMatrix"
        ...
        if(request("ProductReleaseID") <> "" and Not IsNull(request("ProductReleaseID"))) then
			...
			p.Value = clng(request("ProductReleaseID"))
			cm.Parameters.Append p
	...
	do while not rs.EOF
        ...
        if request("OSReleaseName") <> "" then
         ...
         if request("ReleaseName") <> "" then
			if lcase(trim(rs("ReleaseName"))) <> lcase(trim(request("ReleaseName"))) then
				blnInclude = false
			end if
		end if
	
	    dim strTiersByImageDefn : strTiersByImageDefn = ""
        dim strLastImageDefn : strLastImageDefn = ""
        dim iExists : iExists = 0

        set cm = server.CreateObject("ADODB.Command")
	    Set cm.ActiveConnection = cn
	    cm.CommandType = 4
	    cm.CommandText = "usp_Image_ListImageMatrixTierTotals"

        if request("ImageID") <> "" then
            Set p = cm.CreateParameter("@p_ID", 3, &H0001)
	        p.Value = request("ImageID")
            cm.Parameters.Append p

            Set p = cm.CreateParameter("@p_TypeID", 3, &H0001)
	        p.Value = 1
            cm.Parameters.Append p
        else
            Set p = cm.CreateParameter("@p_ID", 3, &H0001)
	    ...
	    Set cm=nothing
	        ...
	        do while not rs2.EOF
                if rs("id") = rs2("id") then  
                    iExists = 1
                    if rs("id") <> strLastImageDefn then
                        if strLastImageDefn = "" then
                            strTierCountRow = strTierCountRow & "<TD nowrap align=center valign=center>"
                        else 
                            strTierCountRow = strTierCountRow & strTiersByImageDefn & "</TD><TD nowrap align=center valign=center>"
                        end if
                        strTiersByImageDefn = ""
                        strLastImageDefn = rs("id")
                    end if
                    if strTiersByImageDefn <> "" then
                        strTierCountRow = strTierCountRow & "<br />"
                    end if
                    strTiersByImageDefn = "<font size=1 face=verdana>Tier " & rs2("Tier") & " = " & rs2("TierCount") & "</font>"
                    strTierCountRow = strTierCountRow & strTiersByImageDefn
                end if
            rs2.movenext
	        loop
	        rs2.Close
        ...
        if iExists = 0 then
            strTierCountRow = strTierCountRow & "<td>&nbsp;</td>"
        else
            strTierCountRow = strTierCountRow & "</td>"
        end if
		...
		if blnInclude then
            ...
            if request("ProductReleaseID") = "" and request("ReleaseName") = "" and request("FileType") = "" then
				strReleaseNameRow = strReleaseNameRow & "<TD><font size=1 face=verdana><a class=""Filter"" href=""javascript: FilterMatrix('&ReleaseName=" & rs("ReleaseName") & "')"">"  & rs("ReleaseName") & "&nbsp;</a></font></td>"
			else
				strReleaseNameRow = strReleaseNameRow & "<TD><font size=1 face=verdana>"& rs("ReleaseName") & "&nbsp;</font></td>"
			end if
            ...
            if request("OSReleaseName") = "" and request("FileType") = "" then
	...
	if blnFound then
        ...
        dim intPowerCordSupported : intPowerCordSupported = 0
        dim intDuckheadPowerCordSupported : intDuckheadPowerCordSupported = 0
        dim intDuckheadSupported : intDuckheadSupported = 0
		strLangSub = ""
		...
		if request("ImageID") <> "" then
			...
			Set cm.ActiveConnection = cn
			cm.CommandType = 4
			cm.CommandText = "usp_Image_ListImagesForDefinition"
			...
			Set cm.ActiveConnection = cn
			cm.CommandType = 4
			cm.CommandText = "usp_Image_ListImagesForProduct"
        ...
        If Not(rs.EOF and rs.BOF) Then
            ...
            dim iColumnCount : iColumnCount = 9
        ...
        ' change localization row color here
        parentRowColor = "E0F7DA"
        colorRow = false
		...
		do while not rs.EOF
            ...
            if rs("Published") then
           	strSKUBGColor = "white"
    		if trim(strlastRegion) <> trim(rs("RegionID")) or (trim(rs("IsParent")) <> "1" and strLastRegionName <> trim(rs("RegionName"))  )  then
				if blnFirst then
					blnFirst = false
					strLastRegion = rs("RegionID")
                    strLastRegionName = rs("RegionName") 
				else
					'Output row 
				'	strSKUBGColor = "cornsilk"
					...
					if blnFoundImages then
                        ...
                        if colorRow then
                            strRegions = strRegions & " bgcolor=" & parentRowColor
                        end if
			...
			if rs("Geo") & "" <> strLastGeo then
				strRegions = strRegions & "<TR bgcolor=gainsboro><TD colspan=" & ImageDefCount + iColumnCount & "><font size=2 face=verdana color=black><b>" & rs("Geo") & "</b></font></TD></TR>"
				strLastGeo = rs("Geo") & "" 
			end if
			...
			for i = lbound(ImageArray) to ubound(ImageArray)
				if trim(ImageArray(i)) = trim(rs("DefinitionID")) then
					...
					strImagesDisplayed=strImagesDisplayed & "," & rs("ID")
                        ...
                        if trim(rs("IsParent")) <> "0" then
						    if isnumeric(rs("Priority")) then
								set rs2 = server.CreateObject("ADODB.recordset")
							    rs2.Open "usp_Image_HideTier4Localization "  & DisplayedID  & ", " & rs("ID") & ", " & rs("DefinitionID") & ", " & rs("RegionID") ,cn,adOpenForwardOnly
							    if rs2.eof and rs2.bof then
								    OutArray(i) = rs("Priority") & ""
								else
									if rs2("curStatus") <> 0 then
										if trim(rs("Priority")) = "0" then
											OutArray(i) = "Ref. Only"
        								elseif rs("Tier") = 11 then
											OutArray(i) = ""
										elseif isnull(rs("Tier")) = false then
											OutArray(i) = "Tier " & rs("Tier")  
										else
											OutArray(i) = "X" 
										end if
									end if
							    ...
							    set rs2 = server.CreateObject("ADODB.recordset")
							    rs2.Open "usp_Image_GetImageProperties4Sub "  & rs("DefinitionID") & ",'" & replace(rs("Priority")," " ,"")  & "'" ,cn,adOpenForwardOnly
							    if rs2.eof and rs2.bof then
								    OutArray(i) = rs("Priority") & ""
                            ...
                            if isnumeric(rs("Priority")) then
							    if trim(rs("Priority")) = "0" then
								    OutArray(i) = "Ref. Only"
							    elseif isnull(rs("Tier")) = false then
								    OutArray(i) = "Tier " & rs("Tier") '"<a target=""_blank"" href=""DeliverableMatrix.asp?ID=" & rs("ID") & "&PINTest=" & request("PINTest") & """>X</a>"  
							    else
								    OutArray(i) = "X"
                                end if
							    intNativeCount = intNativeCount + 1
		...
		'	if strRegionDef = "" then
				...
				strRegionDef = strRegionDef & "<TD " & strKWL & "><font face=verdana size=1>" & rs("Keyboard") & "</font></td><TD><font face=verdana size=1>" & strKWL & "</font></td>"
                
                if iPowerCordSupported = 1 then
                    strRegionDef = strRegionDef & "<TD><font face=verdana size=1>" & rs("PowerCordGEO") & "</font>&nbsp;</td>"
                end if

                if iDuckheadPowerCordSupported = 1 then
                    strRegionDef = strRegionDef & "<TD><font face=verdana size=1>" & rs("DuckheadPowerCordGEO") & "</font>&nbsp;</td>"
                end if

                if iDuckheadSupported = 1 then
                    strRegionDef = strRegionDef & "<TD><font face=verdana size=1>" & rs("DuckheadGEO") & "</font>&nbsp;</td>"
                end if
                
                strRegionDef = strRegionDef & "<TD><font face=verdana size=1>" & rs("RestoreMedia") & "&nbsp;</font></td>"
        ...
        boolRegionsDisplay = False
		for i = lbound(PriorityArray) to ubound(PriorityArray)
			if trim(PriorityArray(i)) <> "" then
                if boolRegionsDisplay <> True then
                    boolRegionsDisplay = True
                    strRegions = strRegions & "<TR"
                    if colorRow then
                        strRegions = strRegions & " bgcolor=" & parentRowColor
                    end if
			...
			if boolRegionsDisplay then
				strRegions = strRegions & "<TD nowrap align=center><font size=1 face=verdana>"	&  OutArray(i) & "</font></td>"
            elseif i=0 then
                boolRegionsDisplay = True
                'for createing the new TR for table
                 strRegions = strRegions & "<TR"
                    if colorRow then
                        strRegions = strRegions & " bgcolor=" & parentRowColor
                    end if
...
<%
	...
	'end if
	strModelRow = strModelRow & "<td bgcolor=gainsboro><font size=1 face=verdana><b>Model </b></font></td><td bgcolor=white rowspan=7 colspan=" & iColumnCount - 1 & "><font size=1 face=verdana><b>Total Images:&nbsp;<label ID=lblImageCount>" & TotalImageCount & "</label></b></font>" & strCountNote & strShowOption & "</td></tr>"
	strOSRow = strOSRow & "<td bgcolor=gainsboro><font size=1 face=verdana><b>OS </b></font></td></tr>"
    strReleaseNameRow = strReleaseNameRow & "<td bgcolor=gainsboro><font size=1 face=verdana><b>Release </b></font></td></tr>"
...
%>
		...
		<%
			...
			strHeader = strTierHeader & "<TD style=""BORDER-TOP: black solid""><font size=1 face=verdana><b>Localization</b></font></TD><TD style=""BORDER-TOP: black solid""><font size=1 face=verdana><b>HP<BR>Code</b></font></TD><TD style=""BORDER-TOP: black solid""><font size=1 face=verdana><b>GM<br>Code</b></font></TD><TD style=""BORDER-TOP: black solid""><font size=1 face=verdana><b>Country<BR>Code</b></font></TD><TD style=""BORDER-TOP: black solid""><font size=1 face=verdana><b>Product<br>DASH</b></font></TD><TD style=""BORDER-TOP: black solid""><font size=1 face=verdana><b>Languages</b></font></TD><TD style=""BORDER-TOP: black solid""><font size=1 face=verdana><b>Keyboard</b></font></TD><TD style=""BORDER-TOP: black solid""><font size=1 face=verdana><b>KWL</b></font></TD>"
            
            if iPowerCordSupported = 1 then
                strHeader = strHeader & "<TD style=""BORDER-TOP: black solid""><font size=1 face=verdana><b>Power Cord</b></font></TD>"
            end if

            if iDuckheadPowerCordSupported = 1 then
                strHeader = strHeader & "<TD style=""BORDER-TOP: black solid""><font size=1 face=verdana><b>Duckhead<BR>Power Cord</b></font></TD>"
            end if

            if iDuckheadSupported = 1 then
                strHeader = strHeader & "<TD style=""BORDER-TOP: black solid""><font size=1 face=verdana><b>Duckhead</b></font></TD>"
            end if
            
            strHeader = strHeader & "<TD style=""BORDER-TOP: black solid""><font size=1 face=verdana><b>Restore<BR>Solution</b></font></TD></TR>"

            Response.Write strHeader
			
		%>
		<%=strRegions%>
		<%=strTotalRows & "<TD colspan=" & iColumnCount & " style=""BORDER-TOP: black solid""><font face=verdana size=1>Image Count</font></td></tr>"%>
        <%=strTierCountRow & "</tr>"%>
		<%
        if false then
			if Tier0Found then
