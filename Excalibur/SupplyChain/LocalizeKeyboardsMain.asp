        ...
        If instr(item, "chkRow" & BaseID) > 0 Then
            ...
            For j = lbound(arSelectedRow) To ubound(arSelectedRow)
                ...
                'If strUserID = 5016 Or strUserID = 8 Or strUserID = 1396 Then
	                ...
	                cm.Open
	                cm.BeginTrans()
	                cm.Execute "usp_InsertLocalizedKeyboard " & clng(request("PVID")) & "," & clng(request("BID")) & ",'" & ScrubSQL(BaseGPG) & "','" & clng(BaseID) & "','" & ScrubSQL(strUserName) & "','" & ScrubSQL(arData(2)) & "','" & ScrubSQL(arData(1)) & "','" & ScrubSQL(arData(3)) & "'," & clng(arData(4)) & ",'" & ScrubSQL(arData(5)) & "'", RowsEffected 
	                If RowsEffected <> 1 Then
	                    cm.RollbackTrans()
	                    ErrorCount = ErrorCount + 1
                        'Response.Write "PVID(" & request("PVID") & ") BID(" & request("BID") & ") baseGPG(" & BaseGPG & ") RootID(" & BaseID & ") CountryCode(" & arData(1) & ") ConfigCode(" & arData(2) & ") UserName(" & strUserName & ")  GeoID(" & arData(4) & ") ParenAVID(" & arData(3) & ")</br>" 
...
'	strImages = ""
	...
	rs.open "spListKeyboardsForLocalization " & clng(request("PVID")) & "," & clng(request("BID")) & ",'"  & ScrubSQL(request("strSeriesSummary")) & "'",cn,adOpenForwardOnly
	LastRootID = ""
	if rs.EOF and rs.BOF then
		...
		do while not rs.EOF
				...
				if rs("AvId") = "" Or IsNull(rs("AvId")) then
				    strRows = strRows & "<TR bgcolor=ivory><TD><input class=""" & trim(rs("RootID")) & """ type=""checkbox"" unchecked id=chkRow" & rs("RootID") & " name=""chkRow" & rs("RootID") & """ value=""" & rs("RootID") & "|" & rs("CountryCode") & "|" & rs("OptionConfig") & "|" & rs("AvParentID") & "|" & rs("GeoID") & "|" & rs("SeriesNum") & """ style=""WIDTH:16;HEIGHT:16""  LANGUAGE=javascript onclick=""return chkRow_onclick()""></TD>"
					NoCount = NoCount + 1
				else
					strCellColor = "ivory"
					YesCount = YesCount + 1
				...
				strBase = strBase &  "<TD width=250> <input style=""width:250; font-size:10; font-family:Verdana"" maxlength=50 type=text id=txtGPGDescription" & rs("RootID") & " name=txtGPGDescription" & rs("RootID") & " value=""" & rs("BaseGPGDescription") & " " &  rs("SeriesNum") & """ onchange=""GpgDescription_onchange(" & rs("RootID") & ")""></TD>"
				...
				strRows = strRows & "<TD width=250><span class=gpgDescription" & rs("RootId") & ">" & rs("BaseGPGDescription") & "&nbsp;" &  rs("SeriesNum") & "</span>&nbsp;" & rs("CountryCode") & "</TD>"
