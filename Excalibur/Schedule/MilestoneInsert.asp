...
Sub SaveData()
	...
	Dim dw, cn, cmd, iRowsChanged, sShowOnReports_YN, prodVersionIDs, rs, strSQL, prodGroupIDs, sItemDescription, sCustomItemDefinition, sRTMitemOSReleaseId
	...
	If Request.Form("cbxISRtmItem") = "on" Then
		sRTMitemOSReleaseId = Request.Form("selItemOSRelease")
		sItemDescription = Request.Form("RTMDescriptionName")
		sCustomItemDefinition = Request.Form("RTMDescriptionName")
	Else 
...
Sub FillOSRelease()
	...
	Set rs = dw.ExecuteCommandReturnRS(cmd)
	Do Until rs.eof
		Response.Write "<OPTION Value=""" & rs("Description") & """>" & rs("Description") & "</OPTION>"
		rs.MoveNext
	Loop
...
<!--
	...
	function selItemOSRelease_onChanged() {
		...
		document.getElementById("RTMDescriptionName").value = "RTM " + selectTxt;
	}
...
<% Call Main() %>
	...
	<table WIDTH="100%" BGCOLOR="cornsilk" BORDER="1" CELLSPACING="0" CELLPADDING="2" bordercolor="tan">
		...
		<tr>
			...
			<td><INPUT type="checkbox" id=cbxISRtmItem name=cbxISRtmItem onclick="return cbxISRtmItem_onclicked()"></td></tr>
		<tr>
	    ...
	    <tr>
			...
			<td><INPUT type="checkbox" id=cbxShowOnReports name=cbxShowOnReports CHECKED></td></tr>
		<tr>
...
<INPUT type="hidden" id="RTMDescriptionName" Name="RTMDescriptionName">
</form>
</BODY>
</HTML>
