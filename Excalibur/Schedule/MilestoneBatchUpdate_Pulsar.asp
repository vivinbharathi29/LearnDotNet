...
Sub Draw()
	...
	Do While Not rs.EOF  
		...
		Response.Write "<INPUT type=hidden id=oldstart-dateselection" & sScheduleDataID & " value='" & sDateStart & "'>"
        Response.Write "<INPUT type='text' size=10 id=DateStart" & sScheduleDataID & "  name=DateStart" & sScheduleDataID & " value='" & sDateStart & "' autocomplete='off' class='dateselection-validate'><!--//&nbsp;<a href=""javascript: cmdDate_onclick('DateStart" & sScheduleDataID & "')""><img ID='picTarget' SRC='../mobilese/today/images/calendar.gif' alt='Choose Date' border='0' WIDTH='26' HEIGHT='21'></a>//--></TD>"
		...
		If NOT bIsMilestone Then
            ...
            Response.Write "<INPUT type=hidden id=oldend-dateselection" & sScheduleDataID & " value='" & sDateStart & "'>"
			Response.Write "<INPUT type='text' size=10 id=DateEnd" & sScheduleDataID & " name=DateEnd" & sScheduleDataID & " value='" & sDateEnd & "' autocomplete='off' class='dateselection-validate'><!--//&nbsp;<a href=""javascript: cmdDate_onclick('DateEnd" & sScheduleDataID & "')""><img ID='picTarget' SRC='../mobilese/today/images/calendar.gif' alt='Choose Date' border='0' WIDTH='26' HEIGHT='21'></a>//--></TD>"
		End If
	...
	Response.Write "<TABLE Width='100%'><TR><TD vAlign=bottom align=right colspan=3><INPUT class='buttonHover1' type=submit value=Save id=submit name=submit>&nbsp;<INPUT class='buttonHover2' type='button' value='Cancel' id=button1 name=button1 onclick='cancel_onClick();'></TD></TR></Table>"
...
function cmdDate_ScheduleResult(FieldID, oldValue) {
    ...
    if (typeof (strID) == "undefined") {
        ...
        if (FieldID.indexOf("DateStart") > -1) {
            MilestoneName = MilestoneName.concat(FieldID.substr(9));  // DateStart
        } else {
            MilestoneName = MilestoneName.concat(FieldID.substr(9));  // DateEnd
        }        
        document.fMain.elements(FieldID).value = strID;
    }
}
...
<BODY onload="onLoad();">
<FORM action="milestonebatchupdate_Pulsar.asp" method="post" id="fMain" name="fMain" onsubmit="return verify();">
