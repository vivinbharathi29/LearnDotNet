...
function cboStatus_onchange() {
	...
	txtStatusText = frmStatus.cboStatus.options[frmStatus.cboStatus.selectedIndex].text;
...
<%
	...
	if request("ProdID") = "" or request("VersionID") = "" then
		...
		rs.Open "spGetCommodityStatus " & clng(request("ProdID")) & "," & clng(request("VersionID")),cn,adOpenForwardOnly
		if rs.EOF and rs.BOF then
			strNumber = ""
		else
			blnFound = true
...
'		strIntegrationTestStatus = replace(replace(replace(replace(strIntegrationTestStatus,"1","Passed"),"2","Failed"),"3","Blocked"),"0","TBD")
...
'		strDeveloperTestStatusName = replace(replace(replace(strDeveloperTestStatus,"1","Passed"),"2","Failed"),"0","TBD")
		...
		'strODMtestStatus = replace(replace(replace(replace(strODMTestStatus,"1","Passed"),"2","Failed"),"3","Blocked"),"0","TBD")
...
'		strWWANtestStatus = replace(replace(replace(replace(strWWANtestStatus,"1","Passed"),"2","Failed"),"3","Blocked"),"0","TBD")
...
'		if trim(strDeveloperTestStatus) = "1" then
'			strDeveloperTestStatusCheck =  "checked"
'		end if
'		if trim(strIntegrationTestStatus) = "1" then
'			strIntegrationTestStatusCheck =  "checked"
'		end if
'		if trim(strODMTestStatus) = "1" then
'			strODMTestStatusCheck = "checked"
'		end if
		strStatusSelected = ""
		strCommentsRequired = ""
...
<% 
	...
	'strFailSubject = "Qualification Status Updated: " & strFailSubject
	strRows = "<TR>"
...
<form id="frmStatus" method="post" action="QualStatusSave.asp?app=PulsarPlus">
    <%else%>
 <form id="frmStatus" method="post" action="QualStatusSave.asp?pulsarplusDivId=<%=Request("pulsarplusDivId")%>">
    <%end if%>
...
<table ID="tabGeneral" WIDTH="100%" BGCOLOR="cornsilk" BORDER="1" CELLSPACING="0" CELLPADDING="2" bordercolor="tan">
	...
	<tr>
		...
		<td colspan=3>
			...
			%>
			<span id=SupportRow style=display:<%=strSupportDisplay%>><INPUT type="checkbox" id=chkDelete name=chkDelete>Completely remove support</span>
			...
			<%
				...
				'if strStatusSelected = "QComplete" then
					strRiskReleaseDisplay = "none"
				'else
				'	strRiskReleaseDisplay = ""
				'end if
			%>
			<Span ID=RiskReleaseRow style="display:<%=strRiskReleaseDisplay%>"><INPUT type="checkbox" <%=strRiskRelease%> id=chkRiskRelease name=chkRiskRelease>Risk Release</Span>
		...
		<td colspan=3>
			<SELECT style="width:100%" id=cboDCR name=cboDCR  LANGUAGE=javascript onkeydown="return combo_onkeydown()" onfocus="return combo_onfocus()" onclick="return combo_onclick()" onkeypress="return combo_onkeypress()" >
				...
				<%
					...
					'rs.Open "spListActionItems " & clng(request("ProdID")) & ",3,0" ,cn,adOpenForwardOnly
	...
	<TR>
		...
		<TD colspan=3>
			<INPUT <%=strSupplyRestricted%> type="checkbox" id=chkSupplyRestricted name=chkSupplyRestricted value="1"> Supply
			<INPUT <%=strConfigurationRestricted%> type="checkbox" id=chkConfigurationRestricted name=chkConfigurationRestricted value="1"> Configuration
	...
	<tr>
		...
		<td colspan=3>
		<%
			rs.Open "spCountBridgesOnOtherVersions " & clng(request("ProdID")) & "," & clng(request("VersionID")),cn,adOpenForwardOnly
			if not(rs.EOF and rs.BOF) then
		...
		<% 
			...
			rs.open "spGetSubassemblyCount "& clng(request("ProdID")) & "," & clng(request("VersionID")),cn,adOpenForwardOnly
			if not (rs.EOF and rs.BOF)then
			...
			rs.Open "spListSubassembliesAllAppropriate " & clng(request("ProdID")) & "," & clng(request("VersionID")),cn,adOpenForwardOnly
			strSubsLoaded = ""
...
'	response.write "WWAN: " & blnRequiresWWANSignoff & "<BR>"
'	response.write "ODM: " & blnRequiresODMSignoff & "<BR>"
'	response.write "MIT: " & blnRequiresMITSignoff & "<BR>"
'	response.write "Developer: " & blnRequiresDeveloperSignoff & "<BR>"
%>
</BODY>
</HTML>
