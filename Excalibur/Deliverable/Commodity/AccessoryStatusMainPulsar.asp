...
var origStatus = "";
var origKitNumber = "";
var origAccessoryDate = "";

$(document).ready(function () {
    origStatus = $("#cboStatus").val();
    origKitNumber = $("#txtKitNumber").val();
    origAccessoryDate = $("#txtAccessoryDate").val();
    origComment = $("#txtComments").val();
    origKitDescription = $("#txtKitDescription").val();

    window.parent.frames["LowerWindow"].enableButton();
    $("#txtRedirect").val("AccessoryStatusMainPulsar.asp?ProdID=" + $("#txtProdID").val() + "&VersionID=" + $("#txtVersionID").val() + "&ProductDeliverableReleaseID=" + $("#txtProdDelRelID").val() + "&RowID=" + $("#txtRowID").val() + "&TodayPageSection=" + $("#txtTodayPageSection").val());
});
...
function SwitchRelease(ProdID, VersionID, ProductDeliverableReleaseID) {
    ...
    else if (origKitNumber != $("#txtKitNumber").val()) {
        isModified = true;
    }
    else if (origAccessoryDate != $("#txtAccessoryDate").val()) {
        isModified = true;
    }
    else if (origComment != $("#txtComments").val()) {
        isModified = true;
    }
    else if (origKitDescription != $("#txtKitDescription").val()) {
        isModified = true;
    }
    
    if (isModified) {
        if (confirm("Do you want to save your changes for this Release?")) {
            $("#txtRedirect").val("AccessoryStatusMainPulsar.asp?ProdID=" + ProdID + "&VersionID=" + VersionID + "&ProductDeliverableReleaseID=" + ProductDeliverableReleaseID + "&RowID=" + $("#txtRowID").val() + "&TodayPageSection=" + $("#txtTodayPageSection").val());
            window.parent.frames["LowerWindow"].cmdOK_onclick();
        }
        else {
            document.location = "AccessoryStatusMainPulsar.asp?ProdID=" + ProdID + "&VersionID=" + VersionID + "&ProductDeliverableReleaseID=" + ProductDeliverableReleaseID + "&RowID=" + $("#txtRowID").val() + "&TodayPageSection=" + $("#txtTodayPageSection").val();
...
<%
	...
	if request("ProdID") = "" or request("VersionID") = "" then
		...
		rs.Open "spGetAccessoryStatusRelease " & clng(request("ProdID")) & "," & clng(request("VersionID")) & "," & clng(ProductDeliverableReleaseID),cn,adOpenForwardOnly
...
<form id="frmStatus" method="post" action="AccessoryStatusSavePulsar.asp?app=PulsarPlus">
    <%else%>
    <form id="frmStatus" method="post" action="AccessoryStatusSavePulsar.asp?pulsarplusDivId=<%=Request("pulsarplusDivId")%>">
        <%end if%>
