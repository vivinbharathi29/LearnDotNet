        ...
        if (typeof (txtSuccess) != "undefined") {
            if ($("#txtSuccess").val() != "0") {
                ...
                if (parent.window.parent.loadDatatodiv != undefined) {
                 ...
                 window.parent.Close($("#txtVersionID").val(), $("#txtDeliverableVersionID").val(), $("#txtSuccess").val(), $("#txtTodayPageSection").val(), $("#txtProductID").val());
                //if (IsFromPulsarPlus()) {
                //    var data = GetQueryStringValue('pageType');
                //    if (data.toLowerCase() == 'developerrequestremoval') {
                //        window.parent.parent.parent.EditCommodityStatus2ReloadCallback($("#txtSuccess").val());
                //    }
                //    ClosePulsarPlusPopup();
                //}
                //else if (IsFromPulsarPlus() == false) {
                //    window.parent.Close($("#txtVersionID").val(), $("#txtDeliverableVersionID").val(), $("#txtSuccess").val(), $("#txtTodayPageSection").val(), $("#txtProductID").val());
                //}
            }
        }
	...
	if strSuccess <> "0" and not (trim(request("cboStatus")) = "0" and request("chkDelete") = "on")then
		...
		for each strElement in SubsAddedArray
			if isnumeric(strElement) and trim(strElement) <> "" then
				cn.Execute "spAddSubassemblyLink " & clng(request("txtID")) & "," & clng(strElement),rowschanged
				if rowschanged <> 1 then
					strSuccess = "0"
					exit for
				end if 
				'See if this is bridged and generate the notification row
				rs.open "spGetSubassemblyBridge " & clng(request("txtID")) & "," &clng(strElement),cn
		...
		for each strElement in SubsRemovedArray
			if isnumeric(strElement) and trim(strElement) <> "" then
				...
				cn.Execute "spRemoveSubassemblyLink " & clng(request("txtID")) & "," & clng(strElement),rowschanged
...
<input type="hidden" id="txtDeliverableVersionID" name="txtDeliverableVersionID" value="<%=request("txtID")%>" />
