...
function VerifySave(){
	...
	if (window.parent.frames["UpperWindow"].Main.txtCode.value=="")
		{
			alert("You must enter a supplier code or click the Cancel button.");
			window.parent.frames["UpperWindow"].Main.txtCode.focus();
			blnSuccess=false;		
		}
	return blnSuccess;
}
	...
	if (VerifySave())
		{
			...
			cmdOK.disabled =true;
			window.parent.frames["UpperWindow"].Main.submit();
		}
