...
<%
	...
	dim CurrentUserName
	dim CurrentUserID
    dim DeliverableCountRTMDraftEdit
    dim ImageCountRTMDraftEdit
    ...
    if currentuserid = "" then
     ...
     'elseif request("txtIsRTMAsDraft") = "1" then
	else	
		cn.begintrans
		...
		p.Value = clng(request("txtProductRTMID"))
		cm.Parameters.Append p
        ...
        if request("txtIsRTMAsDraft") = "1" then
		p.Value = cbool("1")		
        else
        p.Value = cbool("0")		
        end if
        ...
        if strSuccess <> "0" then
            ...
            for i= 1 to 8  
                ...
                if clng(request("txtProductRTMID")) <> "0" then
	    	        p.Value =0
                else
                    p.Value =1
                end if
      ...
      if clng(request("txtProductRTMID")) <> "0"  then
            if  trim(request("chkBIOS")) <> "1" and trim(request("chkPatch"))<>"1" and trim(request("chkRestore")) <> "1" and trim(request("chkFW")) <> "1" and trim(request("chkImages")) <> "1" then
    		        set cm = server.CreateObject("ADODB.Command")
	    	        cm.CommandType =  &H0004
		            cm.ActiveConnection = cn
		            cm.CommandText = "spRemoveProductRTMImageDeliverable"	
	
                    Set p = cm.CreateParameter("@ProductRTMID", 3,  &H0001)
		            p.Value = clng(strSuccess)
		            cm.Parameters.Append p

                    Set p = cm.CreateParameter("@RemoveImage", 3,  &H0001)
		            p.Value = 0
		            cm.Parameters.Append p
    ...
    if clng(request("txtProductRTMID")) <> "0" and trim(request("chkImages")) = "1" then
            if  trim(request("chkBIOS")) <> "1" and trim(request("chkPatch")) <>"1" and trim(request("chkRestore")) <> "1" and trim(request("chkFW")) <> "1" then
    		        set cm = server.CreateObject("ADODB.Command")
	    	        cm.CommandType =  &H0004
		            cm.ActiveConnection = cn
		            cm.CommandText = "spRemoveProductRTMImageDeliverable"	
    ...
    if clng(request("txtProductRTMID")) <> "0" and trim(request("chkBIOS")) = "1" or trim(request("chkPatch")) = "1" or trim(request("chkRestore")) = "1" or trim(request("chkFW")) ="1" then
            if   trim(request("chkImages")) <> "1" then
    		        set cm = server.CreateObject("ADODB.Command")
	    	        cm.CommandType =  &H0004
		            cm.ActiveConnection = cn
		            cm.CommandText = "spRemoveProductRTMImageDeliverable"	
   ...
   DeliverableCountRTMDraftEdit=0
        'Link Patches to RTM
        if strSuccess <> "0" and trim(request("chkPatch")) = "1" then
            ...
            for each strVersion in VersionArray
               ...
               Set p = cm.CreateParameter("@RTMDraftDeliverableRemove", 3, &H0001)
	           if DeliverableCountRTMDraftEdit = 0 then		    
                   p.value = 1
               else
                   p.value=0
               end if
               cm.Parameters.Append p
                ...
                set cm = nothing
                DeliverableCountRTMDraftEdit=DeliverableCountRTMDraftEdit+1
            next
    ...
    DeliverableCountRTMDraftEdit=0
        ...
        DeliverableCountRTMDraftEdit=0
        'Link Firmware(FW) to RTM
        if strSuccess <> "0" and trim(request("chkFW")) = "1" then
            VersionArray = split(request("chkFWList"),",")
            for each strVersion in VersionArray
     ...
     ImageCountRTMDraftEdit=0
        ...
        if strSuccess <> "0" and trim(request("chkImages")) = "1" then
            ...
            for each strImage in ImageArray
               ...
               Set p = cm.CreateParameter("@RTMDraftImageRemove", 3, &H0001)
	           if ImageCountRTMDraftEdit = 0 then		    
                   p.value = 1
               else
                   p.value=0
                end if
            ...
            ImageCountRTMDraftEdit=ImageCountRTMDraftEdit+1
            next
    ...
    'if strSuccess <> "0" then 
    if strSuccess <> "0" and request("txtIsRTMAsDraft") = "0" then
