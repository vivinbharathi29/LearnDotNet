...
$(function(){
	...
	if (returnValue == "1") {
	    window.parent.Close($("#txtID").val(), $("#txtStatusName").val());
	}
});
...
<%
	...
	for each strid in IDArray
        ...
        if trim(request("cboDevStatus")) = "2" and trim(request("txtTypeID")) <> "2" then  'Developer Notification Status Updated
            ...
            response.write "<BR>" & stridarr(0)
            rs.open "spGetDeliverableVersionProductPM " & clng(stridarr(0)),cn
	...
	if trim(request("txtTypeID")) = "2" then 
        stridarr = split(request("txtID"),"_")
        'Test Status Updated
    	'Send Email of the change to the HW PM
        if stridarr(1) = 0 then
		    rs.open "spGetDevProductionReleaseEmailInfo " & clng(stridarr(0)),cn,adOpenStatic
        else
            rs.open "spGetDevProductionReleaseEmailInfoPulsar " & clng(stridarr(1)),cn,adOpenStatic
        end if
...
<INPUT type="hidden" id=txtID name=txtID value="<%=request("txtID")%>">
</BODY>
</HTML>
