...
<!--
    ...
    function AddAV(PVID, BID, CurrentUserId) {
        var strID
            strID = window.parent.showModalDialog("/Excalibur/SCM/avFrame.asp?Mode=add&PVID=" + PVID + "&BID=" + BID, "", "dialogWidth:900px;dialogHeight:700px;edge: Sunken;center:Yes; help: No;resizable: Yes;status: No");
    ...
    function LocalizeAVs(PVID, strSeriesSummary, bMultipleSeriesNumbers, BID, UserName, KMATValue) {
        ...
        var strID;
        var url = "/Excalibur/SCM/LocalizeAVsSeriesNumFrame.asp?Mode=add&PVID=" + PVID + "&BID=" + BID + "&strSeriesSummary=" + strSeriesSummary + "&UserName=" + UserName + "&KMAT=" + KMATValue;
    ...
    function StructureBOM(User, BID, KMAT, BusinessID, PVID) {
        var strID;
        var url = "/Excalibur/SCM/StructureBOMFrame.asp?Mode=add&User=" + User + "&BID=" + BID + "&KMAT=" + KMAT + "&BusinessID=" + BusinessID + "&PVID=" + PVID;
    ...
    function AvActionScorecard(UserId) {
        var strID;
        var url = "/Excalibur/SCM/AvActionScorecardFrame.asp?UserID=" + UserId;
    ...
    function UploadAvData(BID, PVID, UserName) {
        var strID;
        var url = "/Excalibur/SCM/UploadAvDataFrame.asp?PVID=" + PVID + "&BID=" + BID + "&UserName=" + UserName
    ...
    function AddSAs() {
        var strID;
        strID = window.open("/Excalibur/MobileSE/SubAssembly.asp?SAType=223&Business=100&Family=100", "", "width=1100,height=800,toolbar=0,resizable=1,scrollbars=1")
    }
    
    function QuickSearch() {
        var strID;
        var url = "/Excalibur/MobileSE/QuickSearch.asp"
    ...
    function UploadScm(BID) {
        var strID
        strID = window.parent.showModalDialog("/Excalibur/SCM/UploaderFrame.asp?BID=" + BID, "", "dialogWidth:500px;dialogHeight:300px;edge: Sunken;center:Yes; help: No;resizable: Yes;status: No")
        if (strID == null)
            return;
        window.location.reload();
    }
    ...
    function EditKMAT(PVID, BID) {
        var strID
        //window.parent.showModalDialog("/Excalibur/SCM/kmatFrame.asp?Mode=add&PVID=" + PVID + "&BID=" + BID, "", "dialogWidth:500px;dialogHeight:650px;edge: Sunken;center:Yes; help: No;resizable: Yes;status: No")
        modalDialog.open({dialogTitle:'Edit Program Data', dialogURL:"/Excalibur/SCM/kmatFrame.asp?Mode=add&PVID=" + PVID + "&BID=" + BID + "", dialogHeight:700, dialogWidth:550, dialogResizable:true, dialogDraggable:true});
        //window.location.reload();
    }

    function showOrphanedAvReport(BID) {
        var strID
        strID = window.open("/Excalibur/SCM/avOrphansFrame.asp?BID=" + BID, "OrphanedAVReport", "width=450,height=500,toolbar=0,resizable=1")
    }
...
//        }
    ...
    function SelectTab(strStep, blnLoad) {
        ...
        document.cookie = "PMTab=" + strStep + ";expires=" + expireDate.toGMTString() + ";path=/Excalibur/";

        CurrentState = strStep;

        window.location.replace("/Excalibur/pmviewLegacy.asp?ID=<%=PVID%>&Class=<%=sClass%>&List=" + strStep);
    }
    ...
    function AddFavorites(strID) {
        ...
        if (!FoundAt > -1) {
            ...
            //jsrsExecute("/Excalibur/FavoritesRSupdate.asp", myCallback, "UpdateFavs", Array(strFavorites, String(FavCount), txtUser.value));
            ajaxurl = "/Excalibur/FavoritesRSupdate.asp?CurrentUserID=" + txtUser.value + "&FavCount=" + String(FavCount) + "&Favorites=" + strFavorites;
            $.ajax({
                url: ajaxurl,
                type: "POST",
                success: function (data) {
                    if (data == "1") {
                        window.parent.frames("LeftWindow").location.replace("/Excalibur/tree.asp?Prog=1&ID=" + AddingID);
    ...
    function ShowProperties(DisplayedID, FusionRequirements) {
        var strID
        strID = window.parent.showModalDialog("/Excalibur/mobilese/today/programs.asp?Commodity=0&ID=" + DisplayedID + "&Pulsar=" + FusionRequirements, "", "dialogWidth:800px;dialogHeight:700px;edge: Sunken;center:Yes; help: No;resizable: Yes;status: No")
    }
    ...
    function ShowProperties_Product(DisplayedID, Clone, FusionRequirements) {
        ...
        ShowPropertiesDialog("/Excalibur/mobilese/today/programs.asp?HWPM=0&ID=" + DisplayedID + shouldClone + "&Pulsar=" + FusionRequirements, "Product Properties", 980, 800);
    }
    ...
    function ShowCommodityProperties(DisplayedID, Type) {
        var strID
        if (Type == 1)
            strID = window.parent.showModalDialog("/Excalibur/mobilese/today/programs.asp?Commodity=1&ID=" + DisplayedID, "", "dialogWidth:675px;dialogHeight:350px;edge: Sunken;center:Yes; help: No;resizable: Yes;status: No")
        else if (Type == 2)
            strID = window.parent.showModalDialog("/Excalibur/mobilese/today/programs.asp?FactoryEngineer=1&ID=" + DisplayedID, "", "dialogWidth:475px;dialogHeight:250px;edge: Sunken;center:Yes; help: No;resizable: Yes;status: No")
        else if (Type == 3)
            strID = window.parent.showModalDialog("/Excalibur/mobilese/today/programs.asp?Accessory=1&ID=" + DisplayedID, "", "dialogWidth:475px;dialogHeight:250px;edge: Sunken;center:Yes; help: No;resizable: Yes;status: No")
        else if (Type == 4)
            strID = window.parent.showModalDialog("/Excalibur/mobilese/today/programs.asp?HWPM=1&ID=" + DisplayedID, "", "dialogWidth:475px;dialogHeight:250px;edge: Sunken;center:Yes; help: No;resizable: Yes;status: No")
    ...
    function RemoveFavorites(strID) {
        ...
        if (!FoundAt > -1) {
            ...
            txtFavCount.value = String(FavCount);
            txtFavs.value = strFavorites;
            //jsrsExecute("/Excalibur/FavoritesRSupdate.asp", myCallback2, "UpdateFavs", Array(strFavorites, String(FavCount), txtUser.value));
            ajaxurl = "/Excalibur/FavoritesRSupdate.asp?CurrentUserID=" + txtUser.value + "&FavCount=" + String(FavCount) + "&Favorites=" + strFavorites;
            $.ajax({
                url: ajaxurl,
                type: "POST",
                success: function (data) {
                    if (data == "1") {
                        window.parent.frames("LeftWindow").location.replace("/Excalibur/tree.asp?Prog=1");
    ...
    function BrandLink_onClick(ProductBrandID) {
        window.location.replace("/Excalibur/scm/pmviewLegacy.asp?ID=<%=PVID%>&Class=<%=sClass%>&BID=" + ProductBrandID);
    }

    function FeatureRow_OnMouseOver() {
    ...
    function FeatureRow_OnMouseOut() {
        var node = window.event.srcElement;
        while (node.nodeName.toLowerCase() != "tr") {
            node = node.parentElement;
        }
    ...
    function FeatureRow_OnClick(pvid, fcid, bid) {
        //var node = window.event.srcElement;
        //while (node.nodeName.toLowerCase() != "tr") {
        //    node = node.parentElement;
        //}
        ...
        var strID;
        strID = window.parent.showModalDialog("/Excalibur/SCM/catFrame.asp?Mode=edit&PVID=" + pvid + "&FCID=" + fcid + "&BID=" + bid, "", "dialogWidth:500px;dialogHeight:400px;edge: Sunken;center:Yes; help: No;resizable: Yes;status: No")
        document.location.reload();
    }

    function AvRow_OnMouseOver() {
        var node = window.event.srcElement;
        while (node.nodeName.toLowerCase() != "tr") {
            node = node.parentElement;
        }
    ...
    function AvRow_OnClick() {
        var node = window.event.srcElement;
        while (node.nodeName.toLowerCase() != "tr") {
            node = node.parentElement;
        }
        var pvid = $(node).attr('pvid');
        var avid = $(node).attr('avid');        
        var bid = $(node).attr('bid');
        ShowAvDetails(pvid, avid, bid);
    }

    function AvRow_OnMouseDown(ProductVersionID, AvDetailID ,BrandId) {
        if (event.button == 2) {            
            RtClickMenuNew(ProductVersionID, AvDetailID ,BrandId);
            return;
        }
        else
        {
    ...
    function ShowAvDetails(ProductVersionID, AvDetailID, ProductBrandID) {
        var strID;
        //if (txtUser.value == 5016) {
        strID = window.parent.showModalDialog("/Excalibur/SCM/avFrame.asp?Mode=edit&PVID=" + ProductVersionID + "&AVID=" + AvDetailID + "&BID=" + ProductBrandID + "&UserID=" + txtUser.value, "", "dialogWidth:900px;dialogHeight:700px;edge: Sunken;center:Yes; help: No;resizable: Yes;status: No")
        //} else {
        //  strID = window.parent.showModalDialog("/Excalibur/SCM/avFrame.asp?Mode=edit&PVID=" + ProductVersionID + "&AVID=" + AvDetailID + "&BID=" + ProductBrandID, "", "dialogWidth:500px;dialogHeight:700px;edge: Sunken;center:Yes; help: No;resizable: Yes;status: No")
        //}
        window.location.reload();
    }
    ...
    //oPopup.document.createStyleSheet("/Excalibur/style/menu.css");
    ...
    function RtClickMenu(ProductVersionID, AvDetailID ,BrandId) {
        ...
        var menuHeight = (menuItem * 5);
        debugger;
        var topHeight = 0;

        if ((menuHeight + mouseHeight) > screenHeight) {
            topHeight = mouseHeight - menuHeight;
        }
        else if ((mouseHeight - menuHeight) < 0) {
            topHeight = mouseHeight - (menuHeight / 2);
        }
        else {
            topHeight = event.pageY;
        }
       ...
       // oPopup.document.body.innerHTML = window.PopUpMenu.innerHTML;
        oPopup.document.body.innerHTML = $("#PopUpMenu").html();
        ...
        if (response) {         
            for (i = 0; i < checkBoxes.length; i++) {
                if ((checkBoxes[i].id == "chkAv" + ParentCategory) && (checkBoxes[i].checked == true)) {
                    ...
                    strID = window.parent.showModalDialog("/Excalibur/SCM/avDelete.asp?PVID=" + ProductVersionID + "&AVID=" + AvDetailID + "&BID=" + ProductBrandID, "", "dialogTop:0;dialogLeft:0;dialogWidth:1px;dialogHeight:1px;edge: Sunken;center:Yes; help: No;resizable: Yes;status: No");
    ...
    var strID;
    strID = window.parent.showModalDialog("/Excalibur/SCM/avSetStatus.asp?PVID=" + ProductVersionID + "&AVID=" + AvDetailID + "&BID=" + ProductBrandID + "&Status=" + Status, "", "dialogTop:0;dialogLeft:0;dialogWidth:1px;dialogHeight:1px;edge: Sunken;center:Yes; help: No;resizable: Yes;status: No")
    var row = document.getElementById("AV" + AvDetailID);
    ...
    var strID;
    var response = confirm("You are about to Perminently delete all records of the most recent SCM publish. \n Do you want to continue?");
    if (response) {
    strID = window.parent.showModalDialog("/Excalibur/SCM/scmPublishRollback.aspx?PBID=" + ProductBrandID, "", "dialogTop:0;dialogLeft:0;dialogWidth:1px;dialogHeight:1px;edge: Sunken;center:Yes; help: No;resizable: Yes;status: No");
    document.location.reload();
    }
    }
    ...
    var strID;
    strID = window.parent.showModalDialog("/Excalibur/SCM/mrDatesFrame.asp?PVID=" + ProductVersionID + "&BID=" + ProductBrandID + "&AVID=" + AvDetailID, "", "dialogWidth:325px;dialogHeight:600px;edge: Sunken;center:Yes; help: No;resizable: Yes;status: No")
    //document.location.reload();
    }

    function ImportAv(ProductVersionID, ProductBrandID) {
    var strID;
    strID = window.parent.showModalDialog("/Excalibur/SCM/pasteFrame.asp?PVID=" + ProductVersionID + "&BID=" + ProductBrandID, "", "dialogWidth:500px;dialogHeight:410px;edge: Sunken;center:Yes; help: No;resizable: Yes;status: No")
    window.location.reload();
    }

    function CopyScm(ProductVersionID, ProductBrandID) {
    var strID;
    strID = window.parent.showModalDialog("/Excalibur/SCM/copyFrame.asp?PVID=" + ProductVersionID + "&BID=" + ProductBrandID, "", "dialogWidth:500px;dialogHeight:650px;edge: Sunken;center:Yes; help: No;resizable: Yes;status: No")
    window.location.reload();
    }
    ...
    if (AvID == null) {
    sMode = "LinkFrom";
    strID = window.parent.showModalDialog("/Excalibur/SCM/linkFrame.asp?PVID=" + ProductVersionID + "&BID=" + ProductBrandID + "&Function=" + sMode, "", "dialogWidth:500px;dialogHeight:650px;edge: Sunken;center:Yes; help: No;resizable: Yes;status: No")
    }
    else {
    sMode = "LinkTo";
    strID = window.parent.showModalDialog("/Excalibur/SCM/linkFrame.asp?PVID=" + ProductVersionID + "&BID=" + ProductBrandID + "&Function=" + sMode, "", "dialogWidth:500px;dialogHeight:200px;edge: Sunken;center:Yes; help: No;resizable: Yes;status: No")
    }
    ...
    function ValidateData(ProductBrandID, chked) {
        //window.open("/Excalibur/SupplyChain/SCM_PM_Publish_ValidateData.asp?PBID=" + ProductBrandID);
        if (chked){
            ...
            request.open("POST", "/Excalibur/SCM/SCM_PM_Publish_ValidateData.asp", false);
    ...
    function clearAvHistory(ProductBrandID) {
        ...
        if (answer){
            //jsrsExecute("/Excalibur/SCM/rs_ClearHistory.asp", clearAvHistoryCallback, "ClearHistory", String(ProductBrandID));
            ajaxurl = "/Excalibur/SupplyChain/rs_ClearHistory.asp?ProductBrandID=" + ProductBrandID;
            $.ajax({
                url: ajaxurl,
                type: "POST",
                success: function (data) {
    ...
    function SetDefaultDisplay(strList, CurrentUserID) {
        ...
        if (window.confirm("Are you sure you want to make this display list the default display that you see each time you view a product?")) {
            ...
            ajaxurl = "/Excalibur/DefaultProductTabRSUpdate.asp?CurrentUserID=" + CurrentUserID + "&List=" + strList;
            $.ajax({
                url: ajaxurl,
                type: "POST",
                success: function (data) {
    ...
    function DocKitData(KMAT) {
        var strID;
        var url = "/Excalibur/SCM/DocKitDataFrame.asp?Mode=add&KMAT=" + KMAT;
        ...
        //strID = window.parent.showModalDialog(url, "", "dialogWidth:545px;dialogHeight:690px;edge: Sunken;center:Yes; help: No;resizable: Yes;status: No");
        //window.location.reload();
        //alert(KMAT);
    }
    ...
    function FilterSCMByCategory(BusinessID, BID, UserID, PVID) {
        ...
        var url = "/Excalibur/SCM/FilterByCategoryFrame.asp?BusinessID=" + BusinessID + "&BID=" + BID + "&UserID=" + UserID + "&PVID=" + PVID + "&Categories=" + Categories.value;
        ...
        if (retValue != undefined) {
            //alert(PVID);
            window.location.replace("/Excalibur/scm/pmviewLegacy.asp?ID=" + PVID + "&BID=" + BID + "&SCMCategories=" + retValue + "&Class=Arrow1");
        }
    }
    ...
    function FilterPMByCategory(BusinessID, BID, UserID, PVID) {
        ...
        retValue = window.parent.showModalDialog(url, "", "dialogWidth:315px;dialogHeight:475px;edge: Sunken;center:Yes; help: No;resizable: Yes;status: No");
          if (retValue != undefined) {
            window.location.replace("/Excalibur/scm/pmviewLegacy.asp?ID=" + PVID + "&BID=" + BID + "&PMCategories=" + retValue + "&Class=Arrow1");
    ...
    //Modified By:  09/21/2016 - Harris, Valerie - PBI 23434/ Task 24367 - Change dialogs to JQuery dialogs     
    //*****************************************************************
   ...
   function RtClickMenuNew(ProductVersionID, AvDetailID, BrandId) {
        //event.preventDefault();
        var screenHeight = $(document).height(); //$(document).height();
...
If Not rs.EOF Then
    ...
    %>
    <table class="DisplayBar" style="border-radius:0px !important" width="100%" cellspacing="0" cellpadding="2">
        <tr>
            <td valign="top">
                <table>
                    <tr>
...
rs.Close
	...
	Select Case strShowObsolete
        ...
        Case "all"              'all is no longer a default when no cookie is set yet
                                %>
                                <a href="#" onclick="ShowObsolete('active');">Active</a> | <a href="#" onclick="ShowObsolete('obsolete');">
                                    Obsolete</a> | All
                                <%
                        ...
                        <tr>
                            ...
                            <td width="100%" colspan="2">
	                            ...
	                            Select Case Request("SCMCategories")
		                            Case ""
                                %>
                                <a href="#" onclick="FilterSCMByCategory(<%=m_BusinessID%>,<%=m_BrandID%>,<%=m_CurrentUserId%>,<%=PVID%>);">
                                    By Category</a>
                                <%
		                            Case Else
		                              sSCMCategories = Request("SCMCategories")
                                %>
                                <a href="#" style="color: Red;" onclick="FilterSCMByCategory(<%=m_BusinessID%>,<%=m_BrandID%>,<%=m_CurrentUserId%>,<%=PVID%>);">
                                    By Category</a>
                                <%
	                            End Select
                                %>
                        ...
                        <tr>
                            ...
                            <td width="100%" colspan="2">
	                            ...
	                            Select Case Request("PMCategories")
		                            Case ""
                                %>
                                <a href="#" onclick="FilterPMByCategory(<%=m_BusinessID%>,<%=m_BrandID%>,<%=m_CurrentUserId%>,<%=PVID%>);">
                                    By Category</a>
                                <%
		                            Case Else
		                              sPMCategories = Request("PMCategories")
                                %>
                                <a href="#" style="color: Red;" onclick="FilterPMByCategory(<%=m_BusinessID%>,<%=m_BrandID%>,<%=m_CurrentUserId%>,<%=PVID%>);">
                                    By Category</a>
                                <%
	                            End Select
                                %>
...
'##########################################################
    ...
    <span style="font-size: xx-small; font-family: Verdana">
        ...
        <%End If%>
            <a href="/iPulsar/ExcelExport/SCM.aspx?BID=<%=m_BrandID%>&PVID=<%=PVID%>">Export SCM</a> | 
        ...
        <%End If 'bUnpublished %>
        <a href="/Excalibur/SCM/ChangeLog.aspx?ID=<%=PVID%>&BID=<%=m_BrandID%>" target="New">
            Show Change Log</a> | <a href="/Excalibur/SCM/mrDatesView.asp?ID=<%=PVID%>&BID=<%=m_BrandID%>"
        ...
        <!--| <a href="#" onclick="PMGandGPSyExport(<%=PVID%>);">GPSy / PMG Upload</a><span style="color: Red; font-weight: bold; cursor: pointer;">(Beta)</span>-->
        <% End If %>
        <br />
        <br />
        ...
        If bIsPc or bIsPDM Then %>
            ...
            <span style="display: none"><a href="#" onclick="CopyScm(<%=PVID%>, <%=m_BrandID%>);">
                Copy Existing SCM</a> |</span>
        ...
        <p>
            ...
            <span style="font-size: x-small; color: Red; font-weight: bold;">The information on
                ...
                to go to the published SCM</span></p>
        <% 
        End If
...
Else
        %>
    <DIV id=NewMenu1 class="custom-menu" style="display:none"></DIV>
    <UL id=NewMenu class="custom-menu" style="FONT-SIZE: 12px; OVERFLOW: hidden; display:none; BORDER-TOP: #ccc 1px solid; FONT-FAMILY: Verdana; BORDER-RIGHT: #ccc 1px solid; BACKGROUND: #fff; WHITE-SPACE: nowrap; BORDER-BOTTOM: #ccc 1px solid; POSITION: absolute; COLOR: #333; PADDING-BOTTOM: 5px; PADDING-TOP: 3px; PADDING-LEFT: 5px; BORDER-LEFT: #ccc 1px solid; Z-INDEX: 20; PADDING-RIGHT: 0px; border-radius: 1px; width: 120px; font-size: x-small;">
        <li style="cursor:pointer; font-weight:bold; padding-bottom:3px" class="custom-menu-item" onclick="javascript:MenuProperties();">
            <b>Properties</b>
        </li>
        <li style="cursor:pointer; color:dimgray" class="custom-menu-item" onclick="javascript:MenuEditMrDates();">Edit MR Dates</li>
        <li id="spacer">
            <hr width="95%">
        </li>
        <li id="delete" style="cursor:pointer; color:dimgray" class="custom-menu-item" onclick="javascript:MenuDelete();">
            Delete This AV
        </li>
        <li id="Li1">
            <hr width="95%" />
        </li>
        <li id="obsolete" style="cursor:pointer; color:dimgray" class="custom-menu-item" onclick="javascript:MenuSetObsolete();">
            Obsolete This AV
        </li>
        <li id="activate" style="cursor:pointer; color:dimgray" class="custom-menu-item" onclick="javascript:MenuSetActive();">
            Activate This AV
        </li>
        <li id="Li2">
            <hr width="95%" />
        </li>
        <li id="hide" style="cursor:pointer; color:dimgray" class="custom-menu-item" onclick="javascript:MenuSetHidden();">
            Hide This AV (SCM/PM)
        </li>
        <li id="unhide" style="cursor:pointer; color:dimgray" class="custom-menu-item" onclick="javascript:MenuSetActive();">
            Unhide This AV (SCM/PM)
        </li>
    </UL>
        ...
        <table id="Table1" class="Table" width="100%">
            ...
            <thead>
                <tr class="FrozenHeader">
                    ...
                    <th style="background-color: wheat">
                        Global Series Config Plan<br />
                        for End of<br />
                        Manufacturing (PE) Date
                    </th>
                </tr>
            </thead>
            <tbody>
	...
	Do Until rs.EOF
	   If (iFeatureCategoryID <> rs("FeatureCategoryID")) Then
			...
			sFeatureCategory = rs("AvFeatureCategory")
                %>
                <tr class="FeatureCategory" pvid="<%=PVID%>" fcid="<%=iFeatureCategoryID%>" bid="<%=m_BrandID%>">
                    <td colspan="2">
                        <span style="font-weight: bold;">
                            <%=sFeatureCategory%></span>
                        <%If bIsPc Then%>
                        <br />
                        <input type="checkbox" id="chkAll<%=rs("FeatureCategoryID")%>" name="chkAll<%=rs("FeatureCategoryID")%>"
                            style="width: 16; height: 16" language="javascript" onclick="chkAll_onclick()">
                        <input type="button" value="Delete" id="btnDelete<%=rs("FeatureCategoryID")%>" name="btnDelete"
                            class="button2" language="javascript" onclick="DeleteMultipleAvs(<%=PVID%>,<%=m_BrandID%>)">
                        <%End If%>
                    </td>
                    <td onmouseover="return FeatureRow_OnMouseOver()" onmouseout="return FeatureRow_OnMouseOut()"
                        onclick="return FeatureRow_OnClick(<%=PVID%>, <%=iFeatureCategoryID%>, <%=m_BrandID%>)">
                        <%= PrepForWeb(rs("CategoryMarketingDescription"))%>
                    </td>
                    <td colspan="5" onmouseover="return FeatureRow_OnMouseOver()" onmouseout="return FeatureRow_OnMouseOut()"
                        onclick="return FeatureRow_OnClick(<%=PVID%>, <%=iFeatureCategoryID%>, <%=m_BrandID%>)">
                        &nbsp;
                    </td>
                    <td onmouseover="return FeatureRow_OnMouseOver()" onmouseout="return FeatureRow_OnMouseOut()"
                    ...
                    <% If (sProdVersionBSAMFlag = "True") Then %>
                        <td colspan="15" onmouseover="return FeatureRow_OnMouseOver()" onmouseout="return FeatureRow_OnMouseOut()"
                            onclick="return FeatureRow_OnClick(<%=PVID%>, <%=iFeatureCategoryID%>, <%=m_BrandID%>)">
                            &nbsp;
                        </td>
                    <% Else %>
                        <td colspan="13" onmouseover="return FeatureRow_OnMouseOver()" onmouseout="return FeatureRow_OnMouseOut()"
                            onclick="return FeatureRow_OnClick(<%=PVID%>, <%=iFeatureCategoryID%>, <%=m_BrandID%>)">
                            &nbsp;
                        </td>
                    <% End If %>
		...
		If rs.Fields(0).Value & "" <> "" Then
                ...
                <tr class='<%=rs("Status")%>' id="AV<%=rs.Fields(0).Value%>" pvid="<%=PVID%>" avid="<%=rs.Fields(0).Value%>"
                    status="<%=rs("Status")%>" bid="<%=rs("ProductBrandID")%>">
                    <td class="cell" nowrap>
                        <%If bIsPc Then%>
                        <input class='<%=rs("FeatureCategoryID")%>' type='checkbox' id='chkAv<%=rs("FeatureCategoryID")%>'
                            name='chkAv<%=rs("AvDetailID")%>' value='<%=rs("FeatureCategoryID")%>' style="width: 16;
                            height: 16" language="javascript" onclick="return chkAv_onclick()">
                        <%End If%>
                        <%=PrepForWeb(rs("AvNo"))%>
                    </td>
                    <%if rs("GPGDescSysUpdate") = 0 then%>
                    <td class="cell" style="color: Gray" nowrap onmouseover="return AvRow_OnMouseOver()" 
                        onmouseout="return AvRow_OnMouseOut()" onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)"
                        onclick="return AvRow_OnClick()">
                        <%=PrepForWeb(rs("GPGDescription"))%>
                    </td>
                    <%else%>
                    <td class="cell" nowrap onmouseover="return AvRow_OnMouseOver()" onmouseout="return AvRow_OnMouseOut()"
                        onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)" onclick="return AvRow_OnClick()">
                        <%=PrepForWeb(rs("GPGDescription"))%>
                    </td>
                    <%end if %>
                    ...
                    <td class="cell" style="color: Gray" nowrap onmouseover="return AvRow_OnMouseOver()"
                        onmouseout="return AvRow_OnMouseOut()" onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)"
                        onclick="return AvRow_OnClick()">
                        <%=PrepForWeb(rs("MarketingDescription"))%>
                    </td>
                    <%else%>
                    <td class="cell" nowrap onmouseover="return AvRow_OnMouseOver()" onmouseout="return AvRow_OnMouseOut()"
                        onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)" onclick="return AvRow_OnClick()">
                    ...
                    <td class="cell" style="color: Gray" nowrap onmouseover="return AvRow_OnMouseOver()"
                        onmouseout="return AvRow_OnMouseOut()" onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)"
                        onclick="return AvRow_OnClick()">
                        <%=PrepForWeb(rs("MarketingDescriptionPMG"))%>
                    </td>
                    <%else%>
                    <td class="cell" nowrap onmouseover="return AvRow_OnMouseOver()" onmouseout="return AvRow_OnMouseOut()"
                        onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)" onclick="return AvRow_OnClick()">
                    ...
                    <td class="cell" nowrap onmouseover="return AvRow_OnMouseOver()" onmouseout="return AvRow_OnMouseOut()"
                        onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)" onclick="return AvRow_OnClick()">
                        <%=PrepForWeb(sProgramVersion)%>
                    </td>
                    <%if rs("CplBlindSysUpdate") = 0 then%>
                        <td class="cell" style="color: Gray" nowrap onmouseover="return AvRow_OnMouseOver()"
                            onmouseout="return AvRow_OnMouseOut()" onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)"
                            onclick="return AvRow_OnClick()">
                            <%=PrepForWeb(rs("CPLBlindDt"))%>
                        </td>
                    <%else%>
                        <td class="cell" nowrap onmouseover="return AvRow_OnMouseOver()" onmouseout="return AvRow_OnMouseOut()"
                            onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)" onclick="return AvRow_OnClick()">
                    ...
                    <td class="cell" style="color: Gray" nowrap onmouseover="return AvRow_OnMouseOver()"
                        onmouseout="return AvRow_OnMouseOut()" onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)"
                        onclick="return AvRow_OnClick()">
                        <%=PrepForWeb(rs("RTPDate"))%>
                    </td>
                    <%else%>
                    <td class="cell" nowrap onmouseover="return AvRow_OnMouseOver()" onmouseout="return AvRow_OnMouseOut()"
                        onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)" onclick="return AvRow_OnClick()">
                        <%=PrepForWeb(rs("GeneralAvailDt"))%>
                    </td>
                    <%end if %>
                    <%if rs("RasDiscoSysUpdate") = 0 then%>
                        <td class="cell" style="color: Gray" nowrap onmouseover="return AvRow_OnMouseOver()"
                            onmouseout="return AvRow_OnMouseOut()" onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)"
                            onclick="return AvRow_OnClick()">
                            <%=PrepForWeb(rs("RASDiscontinueDt"))%>
                        </td>
                    <%else%>
                        <td class="cell" nowrap onmouseover="return AvRow_OnMouseOver()" onmouseout="return AvRow_OnMouseOut()"
                            onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)" onclick="return AvRow_OnClick()">
                    ...
                    <td class="cell" nowrap onmouseover="return AvRow_OnMouseOver()" onmouseout="return AvRow_OnMouseOut()"
                        onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)" onclick="return AvRow_OnClick()">
                        <%=PrepForWeb(rs("Group1"))%>
                    </td>
                    <td class="cell" nowrap onmouseover="return AvRow_OnMouseOver()" onmouseout="return AvRow_OnMouseOut()"
                        onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)" onclick="return AvRow_OnClick()">
                        <%=PrepForWeb(rs("Group2"))%>
                    </td>
                    <td class="cell" nowrap onmouseover="return AvRow_OnMouseOver()" onmouseout="return AvRow_OnMouseOut()"
                        onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)" onclick="return AvRow_OnClick()">
                        <%=PrepForWeb(rs("Group3"))%>
                    </td>
                    <td class="cell" nowrap onmouseover="return AvRow_OnMouseOver()" onmouseout="return AvRow_OnMouseOut()"
                        onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)" onclick="return AvRow_OnClick()">
                        <%=PrepForWeb(rs("Group4"))%>
                    </td>
                    <td class="cell" nowrap onmouseover="return AvRow_OnMouseOver()" onmouseout="return AvRow_OnMouseOut()"
                        onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)" onclick="return AvRow_OnClick()">
                        <%=PrepForWeb(rs("Group5"))%>
                    </td>
                    <td class="cell" nowrap onmouseover="return AvRow_OnMouseOver()" onmouseout="return AvRow_OnMouseOut()"
                        onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)" onclick="return AvRow_OnClick()">
                        <%=PrepForWeb(rs("IdsSkus_YN"))%>
                    </td>
                    <td class="cell" nowrap onmouseover="return AvRow_OnMouseOver()" onmouseout="return AvRow_OnMouseOut()"
                        onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)" onclick="return AvRow_OnClick()">
                        <%=PrepForWeb(rs("IdsCto_YN"))%>
                    </td>
                    <td class="cell" nowrap onmouseover="return AvRow_OnMouseOver()" onmouseout="return AvRow_OnMouseOut()"
                        onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)" onclick="return AvRow_OnClick()">
                        <%=PrepForWeb(rs("RctoSkus_YN"))%>
                    </td>
                    <td class="cell" nowrap onmouseover="return AvRow_OnMouseOver()" onmouseout="return AvRow_OnMouseOut()"
                        onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)" onclick="return AvRow_OnClick()">
                        <%=PrepForWeb(rs("RctoCto_YN"))%>
                    </td>
                    <% if (sProdVersionBSAMFlag = "True") Then %>
                        <td class="cell" nowrap onmouseover="return AvRow_OnMouseOver()" onmouseout="return AvRow_OnMouseOut()"
                            onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)" onclick="return AvRow_OnClick()">
                            <%=PrepForWeb(rs("BSAMSkus_YN"))%>
                        </td>
                        <td class="cell" nowrap onmouseover="return AvRow_OnMouseOver()" onmouseout="return AvRow_OnMouseOut()"
                            onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)" onclick="return AvRow_OnClick()">
                            <%=PrepForWeb(rs("BSAMBparts_YN"))%>
                        </td>
                    <% End If %>
                    <td class="cell" nowrap onmouseover="return AvRow_OnMouseOver()" onmouseout="return AvRow_OnMouseOut()"
                        onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)" onclick="return AvRow_OnClick()">
                    ...
                    <td class="cell" nowrap onmouseover="return AvRow_OnMouseOver()" onmouseout="return AvRow_OnMouseOut()"
                        onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)" onclick="return AvRow_OnClick()">
                        <%=PrepForWeb(rs("Weight"))%>
                    </td>
                    <td class="cell" nowrap onmouseover="return AvRow_OnMouseOver()" onmouseout="return AvRow_OnMouseOut()"
                        onmousedown="return AvRow_OnMouseDown(<%=PVID%>,<%=rs.Fields(0).Value%>,<%=rs("ProductBrandID")%>)" onclick="return AvRow_OnClick()">
                        <%=PrepForWeb(rs("GSEndDt"))%>
                    </td>
                </tr>
                <%
                iAvCount = iAvCount + 1        
...
Sub PmDrawSkippedRow(rs, iVersions)
    Response.Write "<tr><td colspan=" & 2 + iVersions & ">Row Skipped</td>"
    Response.Write "<td nowrap>" & rs("AvNo") & "</td>"
    Response.Write "<td nowrap>" & rs("SaNo") & "</td>"
    Response.Write "<td nowrap>" & rs("CompNo") & "</td>"
    If (sProdVersionBSAMFlag = "True") Then
...
Sub PmDrawCategoryRow(rs, iVersions)
    If (sProdVersionBSAMFlag = "True") Then
        ...
        Response.Write "<tr class=""MatrixFeatureCategory""><td colspan=" & 24 + iVersions & ">"
    end if
...
If Trim(rs("CompNo_L5") & "") <> "" Then
    ...
    If (sProdVersionBSAMFlag = "True") Then
        Response.Write "</td><td>&nbsp;"
    End If
    Response.Write "</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr>"
    PmDrawComponentRow_L6 rs, iVersions
End If
End Sub
    ...
    <div id="modal_programmatrix" class="content-dialog hide">
        <div class="menu_form_body">
            ...
            <table>
                ...
                <tr <% If Not (bIsPc And ShowReport <> "spb") Then Response.Write "style=""display: none""" %>>
                        <th>Publish:
                    </th>
                    <td>
                        <input type="checkbox" name="chkPublish" title="Publish" onclick="ValidateData(<%= m_BrandID %>)" />
                    </td>
                </tr>
                <tr>
                        <th>New&nbsp;Matrix:
