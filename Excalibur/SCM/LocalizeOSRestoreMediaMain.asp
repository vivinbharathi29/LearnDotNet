...
Sub GetBaseGPG(BaseID)
    ...
    For Each item in Request.Form
        If instr(item, "txtGPGDescription" & BaseID) > 0 Then
            ...
            arGPG = split(BaseGPG,",")
            'Response.Write(arGPG(0))
            GetSelectedRows BaseID, arGPG(0)
        End If
    Next
End Sub
...
Sub GetSelectedRows(BaseID, BaseGPG)
    For Each item in Request.Form
        ...
        If instr(item, "chkRow" & BaseID) > 0 Then
            ...
            'Response.Write(Request.Form(item) & "</br></br>")
            
            'For each item in Request.Form
                    'If instr(item, "CountryCode" & BaseID) > 0 Then
                       'Response.Write(Request.Form(item) & "</br></br>")
                    'End If
            'Next
            ...
            For j = lbound(arSelectedRow) To ubound(arSelectedRow)
                ...
                'If strUserID = 5016 Or strUserID = 8 Or strUserID = 1396 Then
	                ...
	                cm.Open
	                cm.BeginTrans()
	                cm.Execute "usp_InsertLocalizedOSRestoreMedia " & clng(request("PVID")) & "," & clng(request("BID")) & ",'" & ScrubSQL(BaseGPG) & "','" & clng(BaseID) & "','" & ScrubSQL(strUserName) & "','" & ScrubSQL(arData(2)) & "','" & ScrubSQL(arData(1)) & "','" & ScrubSQL(arData(3)) & "'," & clng(arData(4)), RowsEffected 
	                If RowsEffected <> 1 Then
	                    cm.RollbackTrans()
	                    ErrorCount = ErrorCount + 1
                        'Response.Write "PVID(" & request("PVID") & ") BID(" & request("BID") & ") baseGPG(" & BaseGPG & ") RootID(" & BaseID & ") CountryCode(" & arData(1) & ") ConfigCode(" & arData(2) & ") UserName(" & strUserName & ") ParentID(" & arData(3) & ") OSID(" & arData(4) & ") </br>" 
...
function BaseRow_onmouseover(strID){
	...
	document.getElementById("BaseRow" + strID).bgColor="lightsteelblue";	
}

function BaseRow_onmouseout(strID){
	document.getElementById("BaseRow" + strID).bgColor="cornsilk";	
}
...
function BaseRow_onclick(strID){
	...
	if (document.getElementById("ChildRow" + strID).style.display == "")
	    document.getElementById("ChildRow" + strID).style.display = "none";	
	else
		document.getElementById("ChildRow" + strID).style.display="";	
}
...
'	dim strImages
	...
	rs.open "spListOSRestoreMediaForLocalization " & clng(request("PVID")) & "," & clng(request("BID")) & ",'"  & ScrubSQL(request("strSeriesSummary")) & "'",cn,adOpenForwardOnly
	LastRootID = ""
	if rs.EOF and rs.BOF then
		strAllRows = "<tr><td colspan=10><FONT size=1 face=verdana>No OS Restore Media defined for this product.</font></td></tr>"
	else
		dim imagecount
		imagecount=0
		...
		do while not rs.EOF
				...
				if rs("AvId") = "" Or IsNull(rs("AvId")) then
				    strRows = strRows & "<TR bgcolor=ivory><TD><input class=""" & trim(rs("RootID")) & """ type=""checkbox"" unchecked id=chkRow" & rs("RootID") & " name=""chkRow" & rs("RootID") & """ value=""" & rs("RootID") & "|" & rs("CountryCode") & "|" & rs("OptionConfig") & "|" & rs("AvParentID") & "|" & rs("OSID") & """ style=""WIDTH:16;HEIGHT:16""  LANGUAGE=javascript onclick=""return chkRow_onclick()""></TD>"
					NoCount = NoCount + 1
				else
					strCellColor = "ivory"
					YesCount = YesCount + 1
				...
				strBase = strBase &  "<TD width=150>" & rs("AvNo") & "&nbsp;&nbsp;</TD>"
				'strBase = strBase &  "<TD nowrap>" & rs("SKUNumber") & "&nbsp;&nbsp;</TD>"
				strBase = strBase &  "<TD width=350> <input style=""width:350; font-size:10; font-family:Verdana"" maxlength=50 type=text id=txtGPGDescription" & rs("RootID") & " name=txtGPGDescription" & rs("RootID") & " value=""" & rs("BaseGPGDescription") & """ onchange=""GpgDescription_onchange(" & rs("RootID") & ")""></TD>"
				strBase = strBase &  "</tr>" & vbcrlf
				
				strRows = strRows & "<TD width=150>" & rs("AVNo") & "&nbsp;</TD>"
		        strRows = strRows & "<TD>" & rs("Dash") & "</TD>"
		        
				strRows = strRows & "<TD width=250><span class=gpgDescription" & rs("RootId") & ">" & rs("BaseGPGDescription") & "</span>&nbsp;" & rs("CountryCode") & "</TD>"
		...
		if imagecount = 0 then
			strAllRows = "<tr><td colspan=10><font size=1 face=verdana>No active OS Restore Media defined for this product.</font></td></tr>"
		end if
		strAllRows = strAllRows & strBase
...
<BR>
<h3 align="center"><TH align=center>Select OS Restore Media To Localize</TH></h3>
<BR>
...
<b><font size=2 face=verdana>OS Resore Media:</font><BR></b>

		<table id="OSRestoreTable" class="OSRestoreTable" width=100% border=0 cellspacing=0 cellpadding=1 >
			<thead bgcolor=Wheat>
